...
W519's normal type-`$0C` handoff commits slot [12] at `$28F3AC`. W520 invokes the existing `objSlot12`
directly from the object-driver map. On its first call, state 0 clears the TX layer and 66-word work block,
installs the exact `$2254B8`, `$2254F8`, `$225538`, and `$225478` blocks into sprite banks 2 through 5,
and checks the inherited active P1 record. Its score is zero and does not enter the table, so the owed-name byte
remains zero, state advances to 1, the name-entry cue does not post, and neither input arm runs.

One driver call later, the existing `nobody` arm runs `$28F368`: it takes the established teardown, restores the
16-word `$222638` TX palette to bank 0, stages type 8 with child state 2, and queues slot [12]'s own long id for
retirement. The next frame drains both queues and reaches the deliberate boundary before slot [8] at `$25A770`.
...
Sequence list A entry 8 at `$2914E8` selects `$29177C`; the next longword at `$2914EC` is its
`$FFFFFFFF` terminator. W517 exports exactly `$29177C+$42`, covering `$29177C..$2917BD`, plus only
the absent spawn-table longwords `$29053A->$1EBE30` for index `$9E` and `$290546->$1EBE9C` for index
`$A1`. The contiguous script window stops at the adjacent unexported sequence-driver copy at `$2917BE`.

The exact 33-word script is:
...
The 18 picture words resolve in order to:

```text
1EBE30 1EBE9C 1EB5C0 1EB338 1EB728 1EB7DC 1EC3F4 1EB62C
1EB59C 1EB2CC 1EB4C4 1EB47C 1EB554 1EB260 1EB284 1EA8AC 1EA8AC 1EA8AC
```

The first group contains eight records at `$40000800..$40002400`; the second contains ten at
`$38000800..$38002C00`, each stepping by `$400`. `$8000 $0404` delays the first record five driver calls
and spaces the first 15 records by five calls each. `$8000 $1010` spaces the final three repeated `$05` records
by 17 calls. `$8002 $0060` reaches loop count `$60`; `$8003 $0004` loads 97 calls after the final spawn and
completes W372's existing one-node mode-0 `$290E80` resource. The auxiliary state stays 0, its cached handle
is unchanged, and the primary handle clears before `$FFFF`.

`$FFFF` advances cursor 32 to 36, resets the script cursor, and clears all 18 pool records. The list terminator
then changes type 7 to outer state 2. Its existing normal arm queues type `$0F` and type 7's retirement; both
queues drain on the next driver call. The test installs a deliberate boundary throw before slot [15] executes, so
it stops at `$291F66` without entering ending-selection behavior.

No interpreter or other production source changed. Fourteen distinct spawn selections reuse prior exact windows;
only indices `$9E` and `$A1` need sparse declarations. The script abuts W516 and both pointer windows only
abut prior declarations. The three declarations add 74 bytes, producing 780 windows, 446,709 bytes, and 76
overlap pairs. The direct natural integration passes 1/1, both exact registry sentinels pass 2/2, and
`export-tables.py --verify` passes. No full suite, web-asset export, build, commit, or publication ran. W517 is
the first unpublished wave after W516; W521 remains the publication point.
...
Sequence list A entry 6 at `$2914E0` selects `$2916DA`; entry 7 at `$2914E4` fixes the next boundary at
`$291700`. W515 exports exactly `$2916DA+$26`, covering `$2916DA..$2916FF`, plus only six absent
spawn-table longwords. It does not port the script at `$291700`.

The exact 19-word script is:
...
The nine picture words resolve in order to `$1ECAD8 $1EC55C $1EB62C $1EC5EC $1EC070 $1EB848
$1EC244 $1EB434 $1EB47C`. They create one nine-record group at `$30000200..$30002200`, stepping by
`$400`. `$8000 $0000` emits every record on consecutive driver calls. `$8002 $0060` spends the exact wait.
`$8003 $0000` reuses W372's one-node `$290E58` resource and completes its load, wait, free, clear, and
advance path. The earlier auxiliary loader remains idle with its cached freed handle unchanged. `$FFFF` advances
cursor 24 to 28, clears the pool, and reaches deliberate `Unreached` boundary `$291700` with the primary
resource clear.

No interpreter or other production source changed. The script window abuts W514. Six disjoint sparse longwords
for indices `$57`, `$AE`, `$BB`, `$D1`, `$D5`, and `$F8` add the only other required cartridge reads;
indices `$65`, `$74`, and `$59` reuse prior windows. The seven declarations add 62 bytes, producing 775
windows, 446,507 bytes, and 76 overlap pairs. The direct natural integration passes 1/1, both focused exact
registry sentinels pass 2/2, and `export-tables.py --verify` passes. W515 is the fourth unpublished wave after
W511. No full suite, web-asset export, build, commit, or publication ran.

## W514 VERIFIED: SEQUENCE LIST A ENTRY 5

Sequence list A entry 5 at `$2914DC` selects `$291692`; entry 6 at `$2914E0` fixes the next boundary at
`$2916DA`. W514 exports exactly `$291692+$48`, covering `$291692..$2916D9`, plus only ten absent
spawn-table longwords. It does not port the script at `$2916DA`.

The exact 36-word script is:

```text
8000 0000
8001 3000 0200
00D9 00AD 005F 0054 005D 0064 00B1 00A2 0064 0069 004E
8001 2800 0200
00C4 0053 0072 0059 0067 0075 0064 009C 00CF 0005 0005 0005
8002 0060
8003 0000
FFFF
```

The 23 picture words resolve in order to `$1EC67C $1EC04C $1EB554 $1EB3C8 $1EB50C $1EB608
$1EC0DC $1EBEC0 $1EB608 $1EB6BC $1EB2F0`, then `$1EC388 $1EB3A4 $1EB800 $1EB47C
$1EB674 $1EB86C $1EB608 $1EBDE8 $1EC514 $1EA8AC $1EA8AC $1EA8AC`. The first group creates
11 records from `$30000200`; the second creates 12 from `$28000200`; both step by `$400`. `$8000 $0000`
emits every record on consecutive driver calls. `$8002 $0060` spends the exact wait. `$8003 $0000` reuses
W372's one-node `$290E58` resource and completes its load, wait, free, clear, and advance path. `$FFFF`
advances cursor 20 to 24, clears the pool, and reaches deliberate `Unreached` boundary `$2916DA`.

No interpreter or other production source changed. The script window abuts W513. Ten disjoint sparse longwords
for indices `$53`, `$67`, `$69`, `$72`, `$75`, `$9C`, `$AD`, `$C4`, `$CF`, and `$D9` add the only other
required cartridge reads. The eleven declarations add 112 bytes, producing 768 windows, 446,445 bytes, and
76 overlap pairs. The direct natural integration passes 1/1, both focused exact registry sentinels pass 2/2,
and `export-tables.py --verify` passes. W514 is the
third unpublished wave after W511. No full suite, web-asset export, build, commit, or publication ran.

## W513 VERIFIED: SEQUENCE LIST A ENTRY 4

Sequence list A entry 4 at `$2914D8` selects `$29166C`; entry 5 at `$2914DC` selects the next boundary
`$291692`. W513 exports exactly `$29166C+$26`, covering `$29166C..$291691`, plus only the absent
spawn-table longword `$2905C2->$1EC2F8` for index `$0C0`. It does not port the script at `$291692`.
...
8000 0000
8001 3000 0200
0058 0054 005D 00C1 00C0 0062 0005 0005 0005
8002 0060
8003 0000
FFFF
```

The nine picture words resolve to `$1EB458`, `$1EB3C8`, `$1EB50C`, `$1EC31C`, `$1EC2F8`,
`$1EB5C0`, then `$1EA8AC` three times. They create one nine-record group at
`$30000200..$30002200`, stepping by `$400`. `$8000 $0000` emits the first record one driver call after
cursor 16 and all nine on consecutive calls. `$8002 $0060` spends the exact wait. `$8003 $0000` reuses
W372's one-node `$290E58` resource and completes its load, wait, free, clear, and advance path.

The test enters W513 with W512's `$290D54` auxiliary first phase in state 2 at progress `[4,4]`. At relative
driver offset 56 it publishes banner 2 and loads `$290E3A` as a two-node mode-0 second phase. At relative
offset 114, `$FFFF` advances cursor 16 to 20 and the sequence driver clears all nine records. Auxiliary state
4 remains active with both timing-index-4 nodes at progress `[29,29]`, banner 2 live, and descriptor index 4
retained. Inner state and active sub-state remain 1, the primary resource is clear, and the script cursor resets.
The next natural call throws `Unreached` at deliberately unexported `$291692`.

No interpreter or other production source changed. The script window abuts W512 and the one sparse longword
abuts W505's `$2905C6+$04` window. The two declarations add 42 bytes, producing 757 windows, 446,333 bytes, and 76 overlap pairs.
The direct natural integration passes 1/1, and the compact directly affected set including both exact registry
sentinels passes 63/63 with no failures or skips. `export-tables.py --verify` passes. W513 is the second
unpublished wave after W511. No full suite, browser export, build, or publication ran.

## W512 VERIFIED: SEQUENCE LIST A ENTRY 3

Sequence list A entry 3 at `$2914D4` selects `$291604`; entry 4 at `$2914D8` selects the next boundary
`$29166C`. W512 exports exactly `$291604+$68`, covering `$291604..$29166B`, plus only the five absent
spawn-table longwords proved necessary by this script. It does not port the script at `$29166C`.

The exact 52-word script is:
...
The 33 ordered selection words resolve to 29 distinct cartridge art pointers. The three `$8001` records install
packed starts `$30000200`, `$28000200`, and `$20000200`; their exact groups contain 9, 11, and 13
consecutive positions at `$400` bumps. `$8000 $0000` emits the first record one driver call after cursor 12 and
all later records on consecutive calls. `$8002 $00C0` waits at script cursor `$0058` for the exact loop count.

Twenty-four distinct spawn selections reuse prior exact windows. The only new spawn longwords are
`$290402->$1EB338` for index `$050`, `$2904D6->$1EBAAC` for `$085`, `$29059A->$1EC190` for
`$0B6`, `$2905E6->$1EC43C` for `$0C9`, and `$29061E->$1EC634` for `$0D7`.

The late `$8005 $0002 $0004` runs while banner 1 is live. It reuses W372's `$290CE8` and `$290DAE`
descriptor sets and W509's `$225AB8/$225B38` targets. `$2907E2` therefore loads first-phase descriptor
`$290D54` through `$246710`: two mode-0 nodes at palette offsets `$40/$80`, 32 words each, timing index 4,
and the shared black target `$246BB8`. At the same time `$8003 $0000` completes operand 0's existing one-node
`$290E58` primary lifecycle. Eight driver frames after the auxiliary phase loads, the primary handle is free and
`$FFFF` advances cursor 12 to 16. The sequence driver clears all 33 pool records. The auxiliary phase remains
active in state 2 with both nodes at progress 4, banner 1 still live, pending banner 2, and descriptor index 4.
The `$290E3A` second phase has not begun and must not be advanced by broadening W512 beyond `$29166C`.

No interpreter or other production source changed. The script window and five sparse longwords are six disjoint
declarations adding 124 bytes. The ignored local table has 755 windows, 446,291 bytes, and 76 overlap pairs.
The direct natural integration passes 1/1, and the compact natural-integration, slot-7, live-registry, and coverage
set passes 32/32 with no failures or skips. `export-tables.py --verify` passes. No full suite, browser export,
build, publication, commit, or push ran.
...
spawn-table longwords proved necessary by this script. It does not inspect the script at `$291604`.

The exact 50-word script is:

```text
8000 0404
8001 3000 0200
0097 008D 0083 0086 005F 0054 005D 0064 00B1 00A2 0005 0005 0005
8001 2800 0200
00E0 00F2 00FF 00AF 005F 0054 005D 0064 00B1 00A2 0005 0005 0005
8001 2000 0200
00F5 0105 0064 00B1 00A2 0005 0005 0005
8002 0060
8003 0000
FFFF
```

The 34 ordered selection words are
`$097,$08D,$083,$086,$05F,$054,$05D,$064,$0B1,$0A2,$005,$005,$005`, then
`$0E0,$0F2,$0FF,$0AF,$05F,$054,$05D,$064,$0B1,$0A2,$005,$005,$005`, then
`$0F5,$105,$064,$0B1,$0A2,$005,$005,$005`. They resolve in that same order to
`$1EBD34,$1EBBCC,$1EBA64,$1EBAD0,$1EB554,$1EB3C8,$1EB50C,$1EB608,$1EC0DC,$1EBEC0,$1EA8AC,$1EA8AC,$1EA8AC`,
then `$1EC778,$1ECA00,$1ECBD4,$1EC094,$1EB554,$1EB3C8,$1EB50C,$1EB608,$1EC0DC,$1EBEC0,$1EA8AC,$1EA8AC,$1EA8AC`,
then `$1ECA6C,$1ECCAC,$1EB608,$1EC0DC,$1EBEC0,$1EA8AC,$1EA8AC,$1EA8AC`.

The three `$8001` records install packed starts `$30000200`, `$28000200`, and `$20000200`. The plain-word
arm adds `$00000400` after each spawn, so the exact position groups contain 13, 13, and 8 consecutive packed
positions. `$8000 $0404` writes count 4 and reload 4. Since the counter decrements only on a data word, the
first record arrives exactly five script-driver calls after sequence cursor 8 and all remaining records arrive five
frames apart. `$8002 $0060` waits at script cursor `$005A` for the exact loop count.

`$8003 $0000` reuses operand 0's one-node `$290E58` primary resource, waits for its authentic chain to drain,
frees it, and clears the primary handle. This script has no `$8005`, does not rearm the auxiliary loader, and
leaves banner, operands, auxiliary idle state, and W509's cartridge-authentic stale freed handle unchanged. No
unsupported opcode, new production behavior, source-call inventory change, or context-key inventory change
was needed.

The final `$FFFF` advances sequence-list cursor 8 to 12. The sequence driver's actual pool clear removes all
34 records, while inner state and active sub-state remain 1 and the shared script cursor resets to zero. The
next natural call throws `Unreached` at the deliberately unexported `$291604`. W511 adds one 100-byte script
window and six four-byte spawn-table windows: `$2904CE->$1EBA64` for `$083`, `$2904DA->$1EBAD0` for
`$086`, `$2904F6->$1EBBCC` for `$08D`, `$29068A->$1ECA00` for `$0F2`, `$290696->$1ECA6C` for
`$0F5`, and `$2906D6->$1ECCAC` for `$105`. The seven disjoint declarations add 124 bytes, producing 749
windows, 446,167 bytes, and 76 overlap pairs. The direct integration passes 1/1 and the compact natural-integration,
slot-7, registry, and coverage set passes 30/30 with no failures or skips.

## W510 COMMITTED AS `38bf2e6`: SEQUENCE LIST A ENTRY 1

Sequence list A entry 1 at `$2914CC` selects `$29154A`; entry 2 at `$2914D0` selects the next boundary
`$2915A0`. W510 exports exactly `$29154A+$56`, covering `$29154A..$29159F`, plus only the nine absent
spawn-table longwords proved necessary by this script. It does not inspect the script at `$2915A0`.

The exact 43-word script is:
...
The 27 ordered selection words are `$07F,$093,$007,$084,$077,$076,$064,$0E4,$062,$0D8,$0F4,$054`,
then `$0B1,$0A2,$074,$0D4,$054,$0C5,$06B,$059`, then
`$064,$05A,$05B,$059,$005,$005,$005`. They resolve in that same order to
`$1EB9D4,$1EBCA4,$1EA8F4,$1EBA88,$1EB8B4,$1EB890,$1EB608,$1EC808,$1EB5C0,$1EC658,$1ECA48,$1EB3C8`,
then `$1EC0DC,$1EBEC0,$1EB848,$1EC5C8,$1EB3C8,$1EC3AC,$1EB704,$1EB47C`, then
`$1EB608,$1EB4A0,$1EB4C4,$1EB47C,$1EA8AC,$1EA8AC,$1EA8AC`.

The three `$8001` records install packed starts `$30000200`, `$28000200`, and `$20000200`. The plain-word
arm adds `$00000400` after each spawn, so the exact position groups contain 12, 8, and 7 consecutive packed
positions. `$8000 $0000` makes the first record arrive on the first script-driver call after sequence cursor 4;
the zero reload then emits the remaining 26 on consecutive driver frames. `$8002 $00C0` waits at script
cursor `$004C` for the exact loop count.

`$8003 $0000` reuses operand 0's one-node `$290E58` primary resource, waits for its authentic chain to drain,
frees it, and clears the primary handle. This script has no `$8005`, does not rearm the auxiliary loader, and
leaves W509's cartridge-authentic stale freed auxiliary handle unchanged. No unsupported opcode or new
production behavior was needed.
...
`$290652->$1EC808` for `$0E4`, and `$290692->$1ECA48` for `$0F4`. The ten disjoint declarations added
122 bytes, producing its historical 742 windows, 446,043 bytes, and 76 overlap pairs. The direct W510
integration passed 1/1 and its compact natural-integration, slot-7, registry, and coverage set passed 30/30 with
no failures or skips.

## W509 COMMITTED AS `1f1cb60`: SEQUENCE LIST A ENTRY 0 AND `$8005`

Inner state 1 initializes sequence list A at `$2914C8`. Entry 0 is `$2914F0`; entry 1 is `$29154A`.
W509 exports exactly `$2914F0+$5A`, covering `$2914F0..$291549` and ending at the next script without
reading it or another sequence list.
...
8000 2000
8001 3000 0200
008E 007E 0007 007D 0098 008A 0092 0007 0081 0064
8001 2800 0200
0103 009F 008C 0097 007C 0094 008F 004E
8001 2000 0200
0079 0077 0095 0080 005F 00A3 0054 005D
8002 00C0
8003 0000
FFFF
```

Its 25 distinct spawn-table selections resolve as follows:

```text
007 -> 1EA8F4   04E -> 1EB2F0   054 -> 1EB3C8   05D -> 1EB50C
05F -> 1EB554   064 -> 1EB608   077 -> 1EB8B4   079 -> 1EB8FC
07C -> 1EB968   07D -> 1EB98C   07E -> 1EB9B0   080 -> 1EB9F8
081 -> 1EBA1C   08A -> 1EBB60   08C -> 1EBBA8   08E -> 1EBBF0
08F -> 1EBC14   092 -> 1EBC80   094 -> 1EBCC8   095 -> 1EBCEC
097 -> 1EBD34   098 -> 1EBD58   09F -> 1EBE54   0A3 -> 1EBEE4
103 -> 1ECC64
```

Fourteen distinct selections reuse prior exact windows. The eleven absent longwords are index `$79` at
`$2904A6`, `$7C` at `$2904B2`, `$80` at `$2904C2`, `$8C` at `$2904F2`, `$8F` at `$2904FE`,
`$94` at `$290512`, `$95` at `$290516`, `$97` at `$29051E`, `$9F` at `$29053E`, `$A3` at
`$29054E`, and `$103` at `$2906CE`. The three `$8001` groups create 10, 8, and 8 records rooted at
`$30000200`, `$28000200`, and `$20000200`; each spawn increments the packed position by `$400`.
`$8000 $2000` delays the first spawn exactly 32 driver frames, after which reload zero permits one record per
frame. `$8002 $00C0` waits for the exact loop count. `$8003 $0000` reuses W372's one-node `$290E58`
resource, loads it once, waits for it to drain, frees it, and clears its primary resource handle.

W509 proves `$8005 word word` from Build-B's `$290AAC..$290AC4` arm. It calls `$2907C2`, which writes
auxiliary state `$81E108` and operands `$81E10A/$81E10C` only if the state was idle, then always advances
the script cursor by six and loops. The reached `$2907E2` state machine now uses the port's real `$246710`,
`$24681A`, `$246800`, and mode-0 `$246410` production paths instead of optional test hooks. With banner
`$81E106` initially zero, operands `$0001/$0003` skip the first resource phase, publish banner 1, and select
`$290DBA -> $290E1C` from the second table in the same call.

`$290E1C` is an exact two-node mode-0 palette descriptor. Both nodes have fill 0, family 0, length 32 words,
and timing index 3. Node 0 starts at current offset `$0040` and targets `$225AB8`; node 1 starts at `$0080`
and targets `$225B38`. These two exact 64-byte targets are W509's only new resource windows. Both nodes
drain after exactly 32 animation steps, the chain is freed, and auxiliary state returns to zero. Build-B leaves
the freed handle cached at `$81E10E`, so the port deliberately does too. The expected palette images also pin
the cartridge channel step that skips value 16: target `$4210`, RGB 16/16/16, retires as `$4631`, RGB
17/17/17, at the end of the fixed lifecycle.
...
732 windows, 445,921 bytes, and 76 overlap pairs.

## W508 COMMITTED AS `faab91f`: VARIANT 0'S FINAL SCRIPT AND LIST TRANSITION

Variant 0 list entry 4 at `$290F2E` is `$291172`, and the `$FFFFFFFF` terminator follows immediately
at `$290F32`. W508 exports exactly `$291172+$3E`, covering `$291172..$2911AF` and ending at the
`$2911B0` menu code. It does not inspect or expose another variant.
...
8000 0000
8001 4800 0200
0054 004D 0054 0058 0064 00E5 00D2 0065
8001 4000 0200
00E6 004F 004D 005B 0059
8000 0808
0005 0005 0005
8002 0080
8003 0004
...
Its 12 distinct spawn-table selections resolve as follows:

```text
005 -> 1EA8AC   04D -> 1EB2CC   04F -> 1EB314   054 -> 1EB3C8
058 -> 1EB458   059 -> 1EB47C   05B -> 1EB4C4   064 -> 1EB608
065 -> 1EB62C   0D2 -> 1EC580   0E5 -> 1EC82C   0E6 -> 1EC850
```

Only `$D2`, `$E5`, and `$E6` were absent from prior sparse declarations. W508 adds their four-byte
longwords at `$29060A`, `$290656`, and `$29065A`; every other selection reuses W504-W507 windows.
The first `$8001` creates eight records from `$48000200`, and the second creates eight from `$40000200`;
each spawn bumps the packed low word by `$400`. `$8000 $0000` emits the first 13 records on consecutive
driver frames. `$8000 $0808` writes adjacent counter and reload bytes `$08/$08`, so each of the final
three `$0005` records arrives nine driver frames after the previous one. `$8002 $0080` waits for exact
loop count `$80`. `$8003 $0004` selects W372's already exported one-node `$290E80` chain, loads it once,
waits for its node to drain, frees it, and clears the handle. No production source change was needed.
...
The runtime edge remains exact and bounded. Variant 0 list entry 2 at `$290F26` is `$290FE2`, while its
following entry at `$290F2A` is `$2910F6`. Before W506, W505's compact integration threw on the first
`$290FE2` word. W506 exports exactly `$290FE2+$5E`; the window ends at `$291040`, the adjacent variant-1
third script, but does not expose any of that script.
...
8000 0000
8001 4800 0200
007F 0093 0007 0084 0077 0076 0065 0106 0102 0062 006C
8001 4000 0200
00EC 0064 008E 007E 0007 007D 0098 008A 0092 0007 0081 0062
8001 3800 0200
0082 0078 008B 0054 0059 0005 0005 0005
8002 00C0
8003 0000
FFFF
```

Its 26 distinct spawn-table selections resolve as follows:

```text
005 -> 1EA8AC   007 -> 1EA8F4   054 -> 1EB3C8   059 -> 1EB47C
062 -> 1EB5C0   064 -> 1EB608   065 -> 1EB62C   06C -> 1EB728
076 -> 1EB890   077 -> 1EB8B4   078 -> 1EB8D8   07D -> 1EB98C
07E -> 1EB9B0   07F -> 1EB9D4   081 -> 1EBA1C   082 -> 1EBA40
084 -> 1EBA88   08A -> 1EBB60   08B -> 1EBB84   08E -> 1EBBF0
092 -> 1EBC80   093 -> 1EBCA4   098 -> 1EBD58   0EC -> 1EC928
102 -> 1ECC40   106 -> 1ECCD0
```

Indices `$05`, `$54`, `$59`, `$62`, `$64`, and `$65` already had W504/W505 windows, so only twenty new
four-byte longwords are declared. The script contains no unknown opcode: the existing interpreter runs all
31 plain-data selections, the `$C0` wait, operand 0's load/wait/free cycle, and `$FFFF` without a source
...
The runtime edge remains exact and bounded. All three variant-list entry-1 longwords at `$290F22`,
`$290F3A`, and `$290F52` are `$290F8E`. Variant 0's following entry at `$290F26` is `$290FE2`.
Before W505, W504's compact integration threw on the first `$290F8E` word. W505 exports exactly
`$290F8E+$54` and no following script bytes.
...
8000 0000
8001 4800 0200
00C1 00CC 00FF 00AF 0064
8001 4000 0200
009B 00F0 00ED 0061 00A5 010F 0064 00E1 0062
8001 3800 0200
010F 00DA 004F 006E 004B 005F 0054 0059 00C8 0005 0005 0005
8002 00C0
8003 0000
FFFF
```

Its 22 distinct spawn-table selections resolve as follows:

```text
005 -> 1EA8AC   04B -> 1EB284   04F -> 1EB314   054 -> 1EB3C8
059 -> 1EB47C   05F -> 1EB554   061 -> 1EB59C   062 -> 1EB5C0
064 -> 1EB608   06E -> 1EB770   09B -> 1EBDC4   0A5 -> 1EBF2C
0AF -> 1EC094   0C1 -> 1EC31C   0C8 -> 1EC418   0CC -> 1EC4A8
0DA -> 1EC6A0   0E1 -> 1EC79C   0ED -> 1EC94C   0F0 -> 1EC9B8
0FF -> 1ECBD4   10F -> 1ECE14
```

Indices `$05`, `$59`, and `$CC` already had W504 windows, so only nineteen new four-byte longwords are
declared. The test's first position expectation exposed an important existing cartridge detail rather than
a source defect: `$81E104` is the low word of `$81E102`. Each plain-data spawn therefore advances the
next pool record's position by `$400`; `$8001` resets the full long before each group. The production RAM
alias already made this faithful, and W505 only clarifies the field comment and pins all 26 positions.
...
pool clear. Its former `$290FE2` stop is superseded by W506 through W511 above.
...
advance. Its former `$290F8E` stop is superseded by W505 through W511 above.
...
`perFrame25FAA4` is called only when `$813098` is clear, exactly where `$25D57E` makes the cartridge call.
The routine first restores both cursor cells and both ROM-fed labels. While `$813006` is clear, it zeros
`$813072`, reads `$23D16C/$23D17E` only for selectors not equal to `$00FF`, and ORs each raw word into that
aggregate. Direction bits 0/1 deliberately act on D0 from the last joined side whose accessor ran; the
cartridge does not reload the aggregate before those tests. A mode change toggles `$813074` and posts
`$28C6FA`. Any aggregate bit in `$70` sets `$813006`.

The confirm arm copies `$813074` to `$80393A`, posts `$28C6E0`, loads `$813076` with `$20`, and decrements it
on the same frame. `$813077` bit 1 controls whether the selected label is redrawn over the normal pair, which
produces the cartridge blink. Zero sets `$813078` and calls `$240EBC` with D0/D1/D2/D3 =
`$74/$0800/$4/$B`. `$25FAA4` saves and restores D0-D7/A0-A6, so `phase7_25D560` must not clear its tracked
D0 provenance after the call.

The local leaves are exact and bounded: `$25FBF2..$25FC13` draws both streams through `$256F14`, and
`$25FC14..$25FC67` draws one highlighted cursor and stream through `$256F78/$256F14`. Both wrappers reduce
to `txBlock240CF0`; retirement reuses `txPrint240EBC`. The streams stay in `RomWindows` at
`$25FC68+$20`, two adjacent `$10`-byte ranges each ending in `$FF`. Do not replace them with source strings.
