...
with 11, 12, and 8 picture words, followed by `$8002 $00C0`, `$8003 $0000`, and `$FFFF`. Its 31
picture words select 26 distinct spawn-table art pointers. Six indices reuse W504/W505 windows, so W506
exports only the exact script and twenty previously absent four-byte longwords. The window stops at
`$291040`, variant 1's adjacent third script, without exposing it.

No opcode arm changed: the stream contains only behavior already pinned by the interpreter. The natural
object-driver integration reaches 31 simultaneous pool records, preserves every ordered art pointer and all
...
445,585 bytes, and 76 overlap pairs. No full suite, browser export, build, or publication ran at W507's
bounded verification point. W507 was subsequently committed as `f942dbe`.

**W508 COMPLETES VARIANT 0'S FIVE-SCRIPT LIST AND FOLLOWS ITS AUTHENTIC TERMINATOR.** List entry 4
at `$290F2E` points to `$291172`; `$FFFFFFFF` follows at `$290F32`. The exact fifth script spans
`$291172..$2911AF`, is `$3E` bytes, and stops exactly before `$2911B0` menu code:
...
The 16 ordered picture words select 12 distinct cartridge art pointers. Two eight-record position groups
begin at `$48000200` and `$40000200`, with each packed position advancing by `$400`. The initial `$0000`
cadence emits the first 13 records on consecutive driver frames; `$8000 $0808` writes counter and reload
bytes `$08/$08`, spacing the final three records by nine driver frames. Only spawn-table indices `$D2`,
`$E5`, and `$E6` were absent; their exact longwords are `$29060A -> $1EC580`, `$290656 -> $1EC82C`, and
`$29065A -> $1EC850`. Every other selection reuses W504-W507 windows. `$8002 $0080` observes an exact
maximum loop count of `$80`. `$8003 $0004` selects the existing one-node resource at `$290E80`, already
covered by W372's resource window, and performs the established load-once, wait-for-drain, free, and
handle-clear lifecycle. No opcode or terminator arm changed.

The fifth `$FFFF` advances variant 0's list cursor from 16 to 20 and the inter-script clear removes all
fifth-script pool records. The existing `$FFFFFFFF` arm then advances variant 0 into inner state 1, which
initializes sequence list A at `$2914C8`; its first pointer is `$2914F0`. That formerly unexported edge is
covered by W509. The direct expanded integration passes 1/1, and the compact natural-integration, slot-7,
registry, and coverage set passes 30/30 with no failures or skips. W508 adds four disjoint windows and 74
bytes, so table regeneration measures 718 windows, 445,659 bytes, and 76 overlap pairs. No full suite,
browser export, build, or publication ran. W508 was subsequently committed as `faab91f`.

**W509 FOLLOWS SEQUENCE LIST A'S EXACT FIRST SCRIPT AND PROVES `$8005`.** Inner state 1 initializes
sequence list A at `$2914C8`; entry 0 points to `$2914F0` and entry 1 points to `$29154A`. The exact first
script spans `$2914F0..$291549`, is `$5A` bytes, and stops at that next pointer's script:
...
The previously unsupported `$8005 word word` arm is exact Build-B behavior: `$290AAC..$290AC4` calls
`$2907C2` with the two operands, advances the shared script cursor by six regardless of whether the helper
accepts them, and loops inside the interpreter. `$2907C2` arms auxiliary state `$81E108` and stores its
operands only while that state is idle. W509 also replaces the reached auxiliary loader's optional test hooks
with its existing production `$246710`, `$24681A`, `$246800`, and mode-0 `$246410` implementations. For this
script, the absent live banner skips the first `$246710` phase, publishes banner 1, and uses index 3 to resolve
`$290DBA -> $290E1C`. That exact mode-0 descriptor has two 32-word nodes targeting `$225AB8` and `$225B38`,
both with timing index 3. Both drain in 32 animation steps and are freed; auxiliary state returns to idle while
the cartridge-authentic freed handle remains cached. The exact palette result also pins the cartridge's
channel-16 skip: target `$4210` retires as `$4631`, from RGB 16/16/16 to 17/17/17, after the fixed lifecycle.

The 26 ordered picture words select 25 distinct spawn-table indices in three position groups of 10, 8, and 8,
rooted at `$30000200`, `$28000200`, and `$20000200`; every position advances by `$400`. Fourteen distinct
selections reuse prior exact windows. W509 declares only the eleven missing spawn longwords:
`$2904A6 -> $1EB8FC`, `$2904B2 -> $1EB968`, `$2904C2 -> $1EB9F8`, `$2904F2 -> $1EBBA8`,
`$2904FE -> $1EBC14`, `$290512 -> $1EBCC8`, `$290516 -> $1EBCEC`, `$29051E -> $1EBD34`,
`$29053E -> $1EBE54`, `$29054E -> $1EBEE4`, and `$2906CE -> $1ECC64`. `$8000 $2000` delays
exactly 32 frames before the first allocation, then reload zero emits each remaining record on the next driver
frame. `$8002 $00C0` spends the exact `$C0` wait. `$8003 $0000` reuses W372's one-node `$290E58`
resource and performs its established load, wait, free, and handle-clear lifecycle.
...
W510's former natural failure at entry-2 script `$2915A0` is covered by W511. Its direct expanded integration
passed 1/1, and its compact natural-integration, slot-7, registry, and coverage set passed 30/30 with no failures
or skips. The script and nine sparse longwords added ten disjoint windows and 122 bytes, so its historical table
measured 742 windows, 446,043 bytes, and 76 overlap pairs. No full suite, browser export, build, publication,
commit, or push ran during bounded W510; it was subsequently committed as `38bf2e6`.

**W511 FOLLOWS SEQUENCE LIST A'S EXACT THIRD SCRIPT.** Entry 2 at `$2914D0` points to `$2915A0`,
and entry 3 at `$2914D4` points to `$291604`. The exact third script spans `$2915A0..$291603`, is
`$64` bytes, and stops at that next script without reading it:
...
The 34 ordered picture words select 17 distinct spawn-table indices. Eleven distinct selections reuse prior exact
windows. W511 declares only six absent longwords: `$2904CE -> $1EBA64` for index `$83`, `$2904DA ->
$1EBAD0` for `$86`, `$2904F6 -> $1EBBCC` for `$8D`, `$29068A -> $1ECA00` for `$F2`, `$290696 ->
$1ECA6C` for `$F5`, and `$2906D6 -> $1ECCAC` for `$105`. The three `$8001` groups create 13, 13,
and 8 records rooted at `$30000200`, `$28000200`, and `$20000200`; every packed position advances by
`$400`.

`$8000 $0404` arms both adjacent counter bytes. The counter decrements only on data words, so the first record
arrives exactly five script-driver calls after the cursor-8 boundary and every later record is five frames after the
previous one. `$8002 $0060` spends the exact `$60` wait. `$8003 $0000` reuses W372's one-node `$290E58`
resource and completes the established load, wait, free, and handle-clear lifecycle. The script contains no
`$8005`: auxiliary state stays idle, banner and operands remain unchanged, and W509's freed handle value remains
cached. No production behavior or exact source-call/context-key inventory changes.

The final `$FFFF` advances sequence-list A's cursor from 8 to 12, and the sequence driver actually clears all
34 presentation records. Inner state and active sub-state remain 1, and the shared script cursor returns to zero.
The next natural call fails loudly at the intentionally unexported entry-3 script `$291604`; no later script, other
...
**W513 FOLLOWS SEQUENCE LIST A'S EXACT FIFTH SCRIPT.** Entry 4 at `$2914D8` points to `$29166C`,
and entry 5 at `$2914DC` points to the next boundary `$291692`. The exact fifth script spans
`$29166C..$291691`, is `$26` bytes, and stops at that next script without reading it:
...
The nine ordered picture words select seven indices and six distinct art pointers: `$1EB458`, `$1EB3C8`,
`$1EB50C`, `$1EC31C`, `$1EC2F8`, `$1EB5C0`, then `$1EA8AC` three times. Six distinct selections reuse
prior exact windows. W513 declares only `$2905C2 -> $1EC2F8` for index `$C0`. The one `$8001` group
creates nine records at `$30000200..$30002200`, advancing the packed position by `$400`. `$8000 $0000`
makes the first record eligible one driver call after cursor 16 and emits all nine on consecutive calls. `$8002
$0060` spends the exact wait, and `$8003 $0000` completes operand 0's existing one-node `$290E58`
load, wait, free, clear, and advance lifecycle.

W512's two-node `$290D54` auxiliary first phase enters W513 in state 2 at progress `[4,4]`. At W513-relative
driver offset 56 it completes, publishes banner 2, and loads `$290E3A` as the two-node mode-0 second phase.
At relative offset 114, `$FFFF` advances sequence-list cursor 16 to 20 and the sequence driver clears all nine
presentation records. Auxiliary state 4 remains active with both timing-index-4 second-phase nodes at progress
`[29,29]`, banner 2 live, and descriptor index 4 retained. Inner state and active sub-state remain 1, the shared
script cursor returns to zero, and the primary resource handle is clear. No unsupported opcode, production
behavior, source-call inventory, or context-key inventory change was needed.

The exact `$29166C+$26` script abuts W512's window, and the sparse `$2905C2+$04` longword abuts
W505's `$2905C6+$04` longword. Together they add two windows and 42 bytes, so table regeneration measures 757 windows, 446,333 bytes, and
76 overlap pairs. The direct natural integration passes 1/1, the compact directly affected set including both
registry sentinels passes 63/63 with no failures or skips, and `export-tables.py --verify` passes. W513 is the
second unpublished wave after W511. No full suite, browser export, build, or publication ran.
The next natural call fails loudly at intentionally unexported sequence-list A entry 5 script `$291692`; no later
sequence script, other sequence list, variant, ending-selection path, or duplicate-only target was inspected.

**W514 FOLLOWS SEQUENCE LIST A'S EXACT SIXTH SCRIPT.** Entry 5 at `$2914DC` points to `$291692`,
and entry 6 at `$2914E0` fixes the next boundary at `$2916DA`. The exact 36-word script spans
`$291692..$2916D9`, is `$48` bytes, and stops without reading the next script:
...
The 23 ordered picture words select 19 distinct indices and art pointers. Nine distinct selections reuse prior
exact windows. W514 adds only indices `$53`, `$67`, `$69`, `$72`, `$75`, `$9C`, `$AD`, `$C4`, `$CF`,
and `$D9`. The two `$8001` groups create 11 records from `$30000200` and 12 from `$28000200`, each
advancing by `$400`. `$8000 $0000` emits all 23 on consecutive driver calls, `$8002 $0060` spends the exact
wait, and `$8003 $0000` completes operand 0's existing one-node `$290E58` lifecycle. `$FFFF` advances the
sequence-list cursor from 20 to 24, clears all 23 records, and leaves inner state and active sub-state at 1 with
the primary resource and pool clear. No interpreter or other production source change was needed.

The exact `$291692+$48` script abuts W513's script. Its ten sparse longwords are disjoint. The eleven windows
add 112 bytes, so table regeneration and `export-tables.py --verify` measure 768 windows, 446,445 bytes, and
76 overlap pairs. The direct natural integration passes 1/1, both focused exact registry sentinels pass 2/2,
and `export-tables.py --verify` passes. W514 is the third unpublished wave after W511.
...
**W518 FOLLOWS SLOT [15]'S AUTHENTIC NORMAL PATH.** W517's committed type `$0F` reaches the existing
`objSlot15` entry at `$291F66`; W518 registers that existing behavior in the direct natural integration rather
than retranscribing it. State 0 clears all 50 `$20`-byte pool records, resets the cursor and frame counter, arms
`$81E120` drift at `$20`, installs the exact 32-word `$222838` palette into bank 2, sets the `$80` timer, and
raises `$81309A` because the inherited `$813098` gate is nonzero.

The existing 47-entry table at `$291FE2` has 7,744 total delay words and a following `$FFFF` terminator. Its
46 drawable payloads select 35 distinct NUL-terminated cartridge strings, 648 ordered character uses, and a
maximum length of 28. Five drawable entries use the horizontal table at `$2923DA`; 41 use the vertical table at
`$29255A`. The final schedule entry is the non-drawing `$FFFFFFFF` payload in horizontal mode. The natural run
spawns all 47 records, draws on 7,966 frames, reaches at most nine live records, and stops drift at relative call
7,791. By then 43 records have retired at the signed `$7800` bound. Four horizontal records remain frozen: the
stop marker at Y `$F820` and strings `$292334/$292344/$292354` at Y `$4860/$4040/$3820`.

The post-drift timer consumes its exact `$80` calls and loads `$291FD8` at relative call 7,918. Its existing
one-node mode-0 animation posts `$28C186` with D1 zero once, fades all 32 installed bank-2 words to black, drains,
and is freed. Slot [15] reaches state 2 at +8,046. On +8,047 it stages type `$0E`, clears `$81E0DA`, queues its
own id for retirement, and both queues drain. The deliberate next handler throws before executing already ported
slot [14] at `$288C6C`. No slot-[14] behavior, other ending arm, type-7 variant or list, adjacent `$2917BE`
driver copy, duplicate target, or unrelated issue was followed. No production source change was needed.
