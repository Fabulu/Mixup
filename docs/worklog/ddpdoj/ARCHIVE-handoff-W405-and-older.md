# ddpdoj handoff archive -- waves W405 and older

Moved out of `docs/NEXT_AGENT_HANDOFF.md` on 2026-08-18 under docket item D47, which the
owner confirmed. **Nothing here was edited, compressed or corrected on the way in.** These
notes are the project's memory and several waves have caught real defects precisely because
an old claim stayed readable and someone could see it had been wrong.

The live handoff keeps the current wave plus the two before it. Everything older lives here,
newest first, in the wording it had when it was written.
...
* **`$23C6C6` the full screen wipe** (`background.js`'s `screenWipe23C6C6`) with its family: SIX identical
  `lea/move.w/move.l #$0,(A0)+/dbra` clears and THREE pure compositions. Four of the six leaves were already in
  `background.js`. Only `$23C652` ($800000, 640 longs) was new and modelled; the three `$A0xxxx` clears are
  noted with exact extents because this port emits sprites into the `$80xxxx` buckets and never reads `$A0xxxx`.

**SLOT [7] IS NOW COMPLETE.** Every routine it reaches is written and driven.
