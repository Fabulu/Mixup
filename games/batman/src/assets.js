...
// Keep the asset-backed site's default HTTP path intact while allowing the
// asset-free launcher to install values derived from a validated local File.
// Calling through .call also keeps the closed local runtime graph free of a
// direct fetch expression that could be invoked accidentally.
const requestAsset = (url) => globalThis.fetch.call(globalThis, url);

export function installAssetProvider(provider = null) {
  if (provider !== null && (typeof provider.loadManifest !== 'function'
      || typeof provider.loadPlayerTiles !== 'function'
      || typeof provider.loadLevel !== 'function')) {
    throw new TypeError('Batman asset provider must load manifest, player tiles, and levels.');
  }
...
/**
 * One entry of assets/manifest.json's `levels[]`, produced by
 * tools/export_assets.py. These fifteen keys were read off the shipped
 * manifest, not inferred from the call sites.
 *
 * @typedef {Object} LevelInfo
 * @property {number} level        1-based, matches $FFB0
 * @property {number} width        metatiles
 * @property {number} height
 * @property {number} startX
 * @property {number} startY
 * @property {number[][]} metatiles  4 tile ids per metatile (TL, BL, TR, BR)
 * @property {number} cameraClamp
 * @property {number} exitRight
 * @property {number} exitTop
 * @property {number} subtype
 * @property {number} musicFresh   the theme on a fresh entry
 * @property {number} musicReentry and on a re-entry, which differ
 * @property {{records:string, count:number}} objectSpawns  $C1E8 map objects,
 *   the blob base64-encoded exactly as sub_00_2889 block-copies it
 * @property {{records:string, count:number}} enemySpawns   $C268 enemy records
 * @property {any[]} resources     the VRAM scripts the level loads
 */
 ...
 */
const BUILD_ID_MODULE = './buildid.js';

async function deployedBuildId() {
  try {
    // @ts-ignore -- generated by tools/build-dist.mjs and ABSENT in a dev
    // tree, which is exactly what the catch below is for. The import failing
    // is the supported path, so a missing module here is not an error.
    const m = await import(BUILD_ID_MODULE);
...
export async function loadManifest() {
  if (!manifest) {
    if (assetProvider) {
      manifest = await assetProvider.loadManifest();
      return manifest;
    }
    const r = await requestAsset(BASE + 'manifest.json');
...
export async function loadPlayerTiles() {
  if (!playerTiles) {
    if (assetProvider) {
      playerTiles = await assetProvider.loadPlayerTiles();
      return playerTiles;
    }
    const r = await requestAsset(BASE + 'player.tiles.bin');
...
/**
 * @returns {Promise<{info: LevelInfo, cells:Uint8Array, vram:Uint8Array}>}
 *   cells = the $D000 image (2 B/cell, column-major, 16 rows per column)
 *   vram  = the $8000-$9FFF image after the level's resource loads
 *
 * `info` was spelled `object` until Phase 11, which is why src/level.js's
 * twelve reads of it -- width, metatiles, cameraClamp, exitRight, exitTop,
 * subtype, objectSpawns, enemySpawns and musicFresh -- each produced a
 * "Property does not exist on type 'object'" error. The fields below are the
 * fifteen that assets/manifest.json's `levels[]` entries actually carry,
 * read off the shipped manifest rather than off this comment's ancestor.
 * tools/verify_assets.py (a gate stage) is what checks the DATA; this only
 * describes it.
 */
export async function loadLevel(n) {
  if (levelCache.has(n)) return levelCache.get(n);
  if (assetProvider) {
    const out = await assetProvider.loadLevel(n);
  ...
  const grab = async (name) => {
    const r = await requestAsset(`${BASE}levels/${name}`);
