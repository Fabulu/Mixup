...
import {
  E_FLAGS, E_STATE, E_FACING, E_SCREEN_X, E_SCREEN_Y, E_ATTR,
  E_X_HI, E_X_LO, E_Y_HI, E_Y_LO, E_CEIL_SNAP, E_PROBE_DX,
} from './record.js';
...
export function riseTail(state, r) {
  if ((r[E_FLAGS] & 0x01) && (r[1] & 0x40) === 0) {
    const v = u8(r[0x13] - 1);                      // $5BC8
    if (v === 0) r[E_FLAGS] = (r[E_FLAGS] & ~0x01) | 0x02;      // $5BCC: apex
    r[0x13] = v;
    addY(r, -i8(v));                                // $5BE3: Y -= vel
    const coll = probeUp(state, r);                 // sub_01_64FA
    if (coll !== 0) {
      if (coll !== 0xFF) {                          // $5BF0 (64FA never returns $FF)
        r[E_Y_HI] = u8(r[E_Y_HI] + 1);              // $5C01: push down a row
        r[E_Y_LO] = r[E_CEIL_SNAP];                 // snap to the blob's head line
      }
      r[0x13] = 0;                                  // $5C06
      r[E_FLAGS] = (r[E_FLAGS] & ~0x01) | 0x02;     // $5C0B
    }
  }
...
export function fallTail(state, r) {
  ...
  if (res === 0) {
    if ((r[E_FLAGS] & 0x03) === 0) {                // $5C57: ground vanished
      r[E_FLAGS] |= 0x02;
      r[1] &= ~0x60;                                // $5C62
    }
  } else {
    r[0x13] = 0;                                    // $5C6E
    if (r[E_FLAGS] & 0x02) {                        // $5C74: landed from a fall
    ...
    if (state.level.number === 0x0A && r[E_Y_HI] >= 0x14) r[E_ATTR] = 0x80;
  }
...
export function screenTail(state, r) {
  ...
  const y = (r[E_Y_HI] << 8) | r[E_Y_LO];
  r[E_SCREEN_X] = u8((u16(x - state.camera.x) >> 4) + 8);   // sub_00_1172
  r[E_SCREEN_Y] = u8((u16((y & 0x0FFF) - state.camera.y) >> 4) + 0x10);

  if (absDiff8(u8((state.camera.y >> 8) + 4), r[E_Y_HI]) >= 7) return;   // $5CCA
  const lvl = state.level.number;
  if (r[E_STATE] === 0x0B) {                        // $5CD4: projectile draw
    const base = [0x6AF3, 0x6AF5, 0x6AF7, 0x6AF9][r[6] - 1] ?? 0x6AFB;  // $5CDD
    queueDraw(state, ar(state, base + r[E_FACING]), r, 0,  // $5D13 / $5D1A
              lvl === 0x0B || lvl === 0x0E);
    return;
  }
  ...
  if (state.level.bossId === 2 && state.flow.bossHop) {
    queueDraw(state, ar(state, 0x6BA3 + (r[E_FACING] & 1)), r, 0, false);   // $5D2D-$5D47
    return;
  }
  ...
  if (state.level.bossId === 1 && state.flow.bossHop) {
    if ((r[1] & 0x20) || (r[E_FLAGS] & 0x04)) {     // $5D5B / $5D61
      state.flow.bossHop = 0;                       // $5DD7
    } else {
      const id = ar(state, ((r[E_FLAGS] & 0x01) ? 0x6BA5 : 0x6BB3)      // $5D6B-$5D74
                    + absDiff8(0x18, r[E_Y_HI]));          // $5D77-$5D84
      if (r[E_FLAGS] & 0x02) {                      // $5D8D: falling
        r[E_PROBE_DX] = 0x10;                       // $5D95
      ...
      queueDraw(state, id, r, r[E_ATTR], true);     // $5DD1: sub_00_0BAF
      return;
    }
  }
  if ((r[E_FLAGS] & 0x04) && (state.frame & 0x08) === 0) return;   // $5DE1: blink
  const alt = lvl === 4 || lvl === 0x0B || lvl === 0x0E;     // $6078 -> 0BAF
  if (state.flow.paused) {                          // $5DF1: draw, no anim tick
    queueDraw(state, r[6], r, r[E_ATTR], alt);
    return;
  }
  ...
  queueDraw(state, id, r, r[E_ATTR], alt);
}
