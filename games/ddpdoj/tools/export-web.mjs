...
// (`06-impl-pixel-slice.md` Ã‚Â§"What I could not do" item 4: "cannot be trimmed
...
// W28 Ã‚Â§6 MADE: 150 of the 166 exist because they appeared in a 161-frame
...
// `docs/worklog/ddpdoj/35-recon-sprite-atlas.md` Ã‚Â§7 states what it would cost to
...
//            columns `20-recon-scroll-engine.md` Ã‚Â§9.3 called unreachable.  They
...
// exactly one map column (recon Ã‚Â§2).  BOOT LOADS SHARDS 0 AND 1 ONLY; the rest
...
const edition = resolveRuntimeProfile();
requireRuntimeCapability(edition.runtime, 'legacyBundle', 'Web bundle export');
const { profile } = edition;
const revision = profile.revisionIdentity;
const gameId = profile.id.split('/')[0];
...
assertProfileTables(profile, tablesJson);
...
// tilts already use (Ã‚Â§WAVE 12) -- and it takes each table to its FULL EXTENT out
...
//
// AND BOOT MUST NOT GET SLOWER (HANDOVER Ã‚Â§8.8).  So the sheet becomes SHARDS
...
//                (46-diag Ã‚Â§6 priced this at 24 entries / 37.3 KiB and Ã‚Â§10.2 said
...
// TWO tables, not one -- and `src/handlers.js` reads both.  See Ã‚Â§WAVE 81.
...
const HARVEST = Object.freeze([
  ...
  [5, 0x26990e, 70, 8, 70, 0x269b3e,
    ...
    + '(46-diag Ã‚Â§6 priced 24 entries / 37.3 KiB and Ã‚Â§10.2 said it had not found '
  ...
  //     reason and W58 Ã‚Â§2.1b's precedent: the table's own extent is the claim,
  ...
  // it in three places (docs/worklog/ddpdoj/81-impl-fighter-mech-art.md Ã‚Â§1):
  //
  ...
  // template's own field, and W86 Ã‚Â§0.2 is what happens when a list's extent
  ...
  [8, 0x28a51c, 36, 4, 36, 0x28a5ac,
    ...
    + 'pool E and same driver $28A098. $22C6BC..$22C860 step $C. [M] W53 Ã‚Â§6 '
    + 'named these as deliberately absent and W86 Ã‚Â§6.3 as the owner\'s '
  ...
  // census said 58 streams.  [M] THE TABLES HOLD 244, and the gap is W81 Ã‚Â§1.1's
  ...
  [17, 0x292c2a, 120, 4, 120, 0x292e0a,
    ...
    + 'for W58 Ã‚Â§2.1b\'s reason and W84\'s: "the index cannot reach them" is a '
 ...
 *  the ten segment images at `$24ACE8`.  `45-impl-laser-beam.md` Ã‚Â§6 measured
...
const W81_IMMEDIATES = Object.freeze([
  ...
  [15, 0x1735fc,
    ...
    + 'fighter W75 Ã‚Â§3.1 photographed off the board\'s framebuffer'],
  [15, 0x173810,
  ...
  [16, 0x17d480,
    ...
    + 'named at 55 s and 65 s and 80 s in W68 Ã‚Â§6 -- `NO ART $17D480` -- for a '
  ...
  [4, 0x07e8ac,
    ...
    + 'This is W81 Ã‚Â§1.1\'s immediate-vs-table lesson a third time'],
]);
...
// W557. Hibachi A2 object 2 emits this fixed descriptor directly from `$2A47FE`.
const W557_IMMEDIATES = Object.freeze([
...
const SPR_SHARDS = Object.freeze([
  ...
  [15, 'type82', 'THE FIGHTER: type $82\'s body $1735FC (96x88) and its '
    ...
    + 'it arrives on the rung the midboss dies (W75 Ã‚Â§4, W81)'],
  [16, 'type88', 'THE TWIN TURRET: type $88\'s body $17D480, its four-frame '
    ...
    + 'with no picture for any (W80 Ã‚Â§5, W81)'],
...
// the BOARD's, not the port's -- `75-diag` Ã‚Â§3's per-type first..last logic frame
// over a 210-rung ladder of the whole stage. [M, cited W75 Ã‚Â§3] type $10 is on
...
// had the recording as their provenance, which is half of what W28 Ã‚Â§6 named as
...
// nothing in this port can ask for them; they are here so that recon 59 Ã‚Â§6's
...
// nothing even with $28112C ported -- the same trap the banner pictures set.
{
  const popup = [0x20168c];
  for (let i = 0; i < 5; i++) popup.push(romBe32(0x2812d4 + i * 4));
  if (new Set(popup).size !== 6) {
    throw new Error(`W234 the bee popup resolves ${new Set(popup).size} streams, `
      + 'not the digits tile plus five distinct x2 tiles');
  }
  addHudStreamGroup('bee popup', popup,
    '$20168C the digits and $2812D4 five x2 tiles');
}
...
// decision and here is its reason, which is W53 Ã‚Â§1.3's applied one level up:
//   * [M] `50-recon` Ã‚Â§2.4 measured "EIGHT distinct kinds on the port's damage
...
}
{
  ...
  if (scripts.size !== 23 || seen.size !== 269) {
    throw new Error(`the ${entries} effect entries resolve to ${scripts.size} `
      ...
      + `and 269 (reproducing 50-recon-effects Ã‚Â§5.1 exactly). A wrong count `
...
// `55-diag` Ã‚Â§10's W56 shopping list is the 29 addresses ONE scenario reached at
...
// there is no table for this file to walk to an extent.  `55-diag` Ã‚Â§10's W59
...
const STRUCTURE_RANGES = Object.freeze([
  ...
  [0x12c7b0, 0x12d430, 32, '3x32, stride 100. [M] 32 streams, closed by $12D430 '
    ...
    + '55-diag Ã‚Â§2.2 calls this "a 38-frame run $12C7B0..$12D3CC"; [M] it is 32'],
  // ------------------------------------------------------------------ WAVE 66
 ...
 *  `46-diag`'s tank hulls and W81 Ã‚Â§1.3's `$272D7A` are the same lesson; this is
...
// golden terrain"*.  `[cited: W68 Ã‚Â§5.2]` named the cause as bucket 2's five
// missing streams and `[cited: W75 Ã‚Â§3.4]` tied one of them, `$232578`, to the
...
/** entry 3 of the same array -- internal stage 3 / human Stage 4, all SEVEN
 *  cells of which the W396 arm below harvests. Here it is only the BOUND. */
...
// ------------------------------------------------------------------ WAVE 396
//
// W211 harvested ONE CELL of internal stage index 3's table -- `$2622D6 + 5*4`,
// the element script 0 requests at clock 0 -- and left the other six entries to
// `src/background.js`'s `unreached`. W394/W395 then ported and harvested the
// whole of internal stage 2, so this was the last BGELEM table in the game with
// a subset arm. W396 ported the six and harvests all seven here.
//
// THE EXTENT IS THE POINTER ARRAY'S, the same bound the arm above uses one cell
// along: `$262302` entry 3 is `$2622D6` and entry 4 is `$2622F2`, so
// `(to - from) / 4` = SEVEN. Both cells are asserted below, so the table's
// length is the cartridge's statement and not a count typed into this file.
//
// **HOW THIS ARM DIFFERS FROM THE STAGE-2 ARM ABOVE**, each difference a decoded
// byte rather than an analogy:
//
//   * THE EMITTER IS THE OTHER ONE. Internal stage 2's fourteen updaters end
//     `4EF9 0023DEFC` (bucket 1); all SEVEN of these end `4EF9 0023DF2A`
//     (bucket 2) at `upd+$2C`/`upd+$2E`. The field is checked here for the same
//     reason it is checked there -- W394 found that aliasing a near-identical
//     updater would have put a whole stage in the wrong sprite bucket with
//     every test still green -- but the CONSTANT is the opposite one, and a row
//     copied from the block above would be wrong in exactly that byte pair.
//   * `bge.w`, NOT `bgt.w`. All seven carry `6C00` at `upd+$0C` where internal
//     stage 2's fourteen carry `6E00`. Same displacement `$0006` in both, so
//     TRAP 19 applies: one byte, and it decides whether an element survives a
//     true sum of exactly 0. Every row here is therefore `lbge`.
//   * SEVEN ENTRIES, SIX DISTINCT STREAMS. The stage-2 arm refuses unless the
//     distinct count EQUALS the entry count. That rule is false here and
//     asserting it would assert a claim the cartridge contradicts: `$262FC8`
//     (id 0) and `$26301A` (id 6) write the same `$2B01D0` descriptor, the same
//     `$3520` Y and the same `$262FE6` updater, and differ only in their last
//     instruction -- `1D7C 0016 000D` (`move.b #$16,($D,A6)`) against
//     `3D7C 0056 000C` (`move.w #$56,($C,A6)`, TRAP 3: one word literal over
//     two byte fields). So the invariant enforced below is the STRUCTURAL one
//     -- rows that share art must share their updater -- plus the measured fact
//     that exactly one such pair exists.
//   * NOT ONE ENTRY IS COMPLEX. Internal stage 2's id 7 installs `$2629AE`,
//     which reverse-walks the 32-pair table `$262A4C` and hides 64 streams
//     behind one handler. All seven rows here are the plain
//     `2D7C <data> 0010` shape with the `4E75` at `ctor+$1C`, so this arm
//     harvests at most one stream per row.
//
// `data` is a SPRITE-MASK offset and is resolved by `romExtent` against
// `sprmask`, never against `maincpu.bin`.
const STAGE4_BGELEM_END = 0x2622f2;
{
  const handlers = BGELEM_HANDLERS.filter((h) => h.stage === 3);
  // THE EXTENT, STATED BY THE CARTRIDGE.
  const from = romBe32(BGELEM_TABLE_PTRS + 3 * 4);
  const to = romBe32(BGELEM_TABLE_PTRS + 4 * 4);
  if (from !== STAGE4_BGELEM_TABLE || to !== STAGE4_BGELEM_END) {
    throw new Error(`the per-stage handler-table array $${
      BGELEM_TABLE_PTRS.toString(16)} names $${from.toString(16)} for internal `
      + `stage 3 and $${to.toString(16)} for stage 4; this file expects $${
        STAGE4_BGELEM_TABLE.toString(16)} bounded by $${
        STAGE4_BGELEM_END.toString(16)}. The extent of the art below is that `
      + 'pair, not a typed-in count.');
  }
  if ((to - from) / 4 !== handlers.length) {
    throw new Error(`internal stage 3's handler table is $${
      (to - from).toString(16)} bytes = ${(to - from) / 4} entries and `
      + `src/background.js carries ${handlers.length} \`stage: 3\` rows. The `
      + 'cartridge decides how many elements the stage has; a short registry '
      + 'ships a subset of the terrain and a long one indexes the next stage.');
  }
  // ---------------------------------------------------------------- PASS ONE
  // THE TABLE, DECODED OUT OF THE IMAGE ALONE. Nothing here reads the registry,
  // so these refusals stand even if `src/background.js` is wrong in exactly the
  // way the pass below would otherwise hide.
  const romCtor = [], romData = [], romUpd = [];
  for (let i = 0; i < handlers.length; i++) {
    const c = romBe32(STAGE4_BGELEM_TABLE + i * 4);
    romCtor.push(c);
    // $262FC8 `2D7C 002B01D0 0010` -- move.l #imm,($10,A6). `0014` at +$6 would
    // be the Y constant (`3D7C` at +$8 writes that), a different field.
    if (romBe16(c) !== 0x2d7c || romBe16(c + 6) !== 0x0010) {
      throw new Error(`internal stage 3 background element ${i}: $${
        c.toString(16)} is not \`move.l #imm,($10,A6)\` -- it reads ${
        romBe16(c).toString(16)} ... ${romBe16(c + 6).toString(16)}`);
    }
    // $262FD6 `2D7C 00262FE6 0008` -- move.l #upd,($8,A6), the third
    // instruction. The registry's `upd` is read out of THIS, never assumed from
    // ctor+$1E, because ids 0 and 6 prove a constructor can install an updater
    // that is not the code following it.
    if (romBe16(c + 0x0e) !== 0x2d7c || romBe16(c + 0x14) !== 0x0008) {
      throw new Error(`internal stage 3 background element ${i}: $${
        (c + 0x0e).toString(16)} is not \`move.l #upd,($8,A6)\` -- it reads ${
        romBe16(c + 0x0e).toString(16)} ... ${romBe16(c + 0x14).toString(16)}`);
    }
    // TRAP 5: the `4E75` sits AT ctor+$1C, so the unit is $1E bytes. A
    // constructor that ran on past here would write fields this arm never read.
    if (romBe16(c + 0x1c) !== 0x4e75) {
      throw new Error(`internal stage 3 background element ${i}: $${
        c.toString(16)} does not close with \`4E75 rts\` AT ctor+$1C -- it `
        + `reads ${romBe16(c + 0x1c).toString(16)}. The five-instruction `
        + 'constructor is what makes `data` readable out of ctor+2.');
    }
    romData.push(romBe32(c + 2));
    romUpd.push(romBe32(c + 0x10));
  }
  // SHARED ART MEANS A SHARED UPDATER. `elemConstruct` writes ($10,A6) once and
  // nothing else ever writes it, so two entries carrying the same descriptor are
  // the SAME element drawn with a different `kind`; on this table they are also
  // literally the same `move.l #upd,($8,A6)` immediate. Two entries that shared
  // art WITHOUT sharing an updater would be two different behaviours reading one
  // picture, which is the shape a mis-transcribed descriptor takes.
  const byData = new Map();
  for (let i = 0; i < romData.length; i++) {
    if (!byData.has(romData[i])) byData.set(romData[i], []);
    byData.get(romData[i]).push(i);
  }
  for (const [d, ids] of byData) {
    if (ids.length === 1) continue;
    if (new Set(ids.map((i) => romUpd[i])).size !== 1) {
      throw new Error(`internal stage 3 entries ${ids.join('/')} all install $${
        d.toString(16)} through \`move.l #imm,($10,A6)\` but not one updater (${
        ids.map((i) => '$' + romUpd[i].toString(16)).join('/')}) -- $${
        ids.map((i) => romCtor[i].toString(16)).join(' $')}. On this table `
        + 'shared art is one element with two `kind` bytes, never two elements.');
    }
  }
  // ...and EXACTLY ONE such pair exists: entries 0 and 6, `$2B01D0`, updater
  // `$262FE6`. MEASURED, and stated as a number so that a seventh distinct
  // descriptor appearing (or a sixth vanishing) is a refusal rather than a
  // quietly different harvest.
  if (handlers.length - byData.size !== 1) {
    throw new Error(`internal stage 3's table $${
      STAGE4_BGELEM_TABLE.toString(16)} has ${handlers.length} entries and ${
      byData.size} distinct descriptors (${
      [...byData.keys()].map((d) => '$' + d.toString(16)).join(' ')
    }); the cartridge has exactly ONE duplicate pair here -- $2B01D0, entries 0 `
      + 'and 6, whose constructors $262FC8 and $26301A differ only in the `kind` '
      + 'instruction, $262FDE `1D7C 0016 000D` against $263030 `3D7C 0056 000C`. '
      + 'Any other count means the harvest is not the table.');
  }

  // ---------------------------------------------------------------- PASS TWO
  // THE REGISTRY, AGAINST WHAT PASS ONE DECODED.
  let added = 0, already = 0;
  const seen = new Set();
  for (let i = 0; i < handlers.length; i++) {
    const h = handlers[i];
    if (h.id !== i) {
      throw new Error(`internal stage 3 row ${i} carries id ${h.id}; these rows `
        + 'are indexed by op $10\'s id through this very table');
    }
    if (romCtor[i] !== h.ctor) {
      throw new Error(`internal stage 3 background element ${i}: the `
        + `cartridge's own table $${STAGE4_BGELEM_TABLE.toString(16)} names $${
          romCtor[i].toString(16)} and src/background.js's row ${i} is $${
          h.ctor.toString(16)}`);
    }
    if (romData[i] !== h.data) {
      throw new Error(`internal stage 3 background element ${i} ($${
        h.ctor.toString(16)}): the cartridge writes $${romData[i].toString(16)} `
        + `into ($10,A6) and src/background.js says $${h.data.toString(16)}. `
        + 'The port would draw one picture and simulate another.');
    }
    if (romUpd[i] !== h.upd) {
      throw new Error(`internal stage 3 background element ${i} ($${
        h.ctor.toString(16)}): the cartridge writes $${romUpd[i].toString(16)} `
        + `into ($8,A6) and src/background.js says $${h.upd.toString(16)}. `
        + 'The updater is what `elemDriver` looks the row up BY, so a wrong one '
        + 'runs another element\'s despawn test against this one\'s slot.');
    }
    // $26319E+$2C `4EF9 0023DF2A` -- the updater's tail jump. BUCKET 2, against
    // internal stage 2's $23DEFC. This is the byte pair that makes these seven
    // rows their own family rather than aliases of $262B6A.
    if (romBe16(h.upd + 0x2c) !== 0x4ef9 || romBe32(h.upd + 0x2e) !== h.emit
        || h.emit !== 0x23df2a) {
      throw new Error(`internal stage 3 background element ${i}: updater $${
        h.upd.toString(16)} does not end \`4EF9 $23DF2A\` -- it reads ${
        romBe16(h.upd + 0x2c).toString(16)} $${
        romBe32(h.upd + 0x2e).toString(16)} against registry $${
        (h.emit >>> 0).toString(16)}. A row aliased onto internal stage 2's `
        + 'updater would put this stage\'s terrain in the wrong sprite bucket.');
    }
    seen.add(h.data);
    if (streams.has(h.data)) { already++; continue; }
    streams.set(h.data, romExtent(h.data));  // throws unless a stream start
    shardOfStream.set(h.data, STRUCT_SHARD);
    added++;
  }
  harvested += added; harvestAlready += already;
  harvestReport.push({ shard: STRUCT_SHARD, base: STAGE4_BGELEM_TABLE,
    entries: handlers.length, stride: 4, runsTo: handlers.length,
    endsAt: STAGE4_BGELEM_TABLE + handlers.length * 4,
    distinct: seen.size, added, already,
    why: 'W396 INTERNAL STAGE 3 (human Stage 4): all 7 elements\' own sprites, '
      + 'six distinct streams (ids 0 and 6 share $2B01D0), extent taken from '
      + 'the per-stage pointer array $262302 (entry 3 $2622D6 bounded by entry '
      + '4 $2622F2) and each descriptor read out of its own `move.l '
      + '#imm,($10,A6)`. W211 shipped id 5 alone; the other six drew the W86 '
      + 'BLACK TERRAIN' });
  console.log(`  internal stage-3 background elements: ${handlers.length} `
    + `handlers, ${seen.size} distinct streams, ${added} new -- STAGE 4's `
    + 'TERRAIN');
}

// ------------------------------------------------------------------ WAVE 397
// (1g.5) INTERNAL STAGE INDEX 4, HUMAN STAGE 5 -- **THE LAST BGELEM TABLE**,
// and the fifth and final arm of this family. With it every constructor the
// cartridge names in any of the five per-stage tables has its picture in the
// sheet: 13 + 8 + 14 + 7 + 4 = 46 rows over 5 tables.
//
// `$2622F2`'s four elements -- `$3053A0 $305D04 $307388 $31975C` -- were
// harvested by nothing and ported by nothing, so all four were the W86 BLACK
// TERRAIN a fifth time.
//
// **THE BOUND IS THE POINTER ARRAY'S OWN BASE, AND THE FOUR ARMS ABOVE CANNOT
// SUPPLY IT.** Each of them took `$262302 + n*4` bounded by `$262302 + (n+1)*4`.
// This is entry 4 and there is no entry 5: `$262316` is the first instruction of
// the slot-clear routine (`41F9 008131C8`, `lea $8131C8,A0`). So the table ends
// where the ARRAY begins, and the array's address is stated by the code that
// reads it rather than typed into this file:
//
//     $262328  41FA FFD8   lea (-$28,PC),A0
//
// TRAP 4 -- the target is the EXTENSION WORD's address plus the displacement,
// `$26232A - $28 = $262302`. That instruction is decoded below and the harvest
// refuses if it stops naming `$262302`, which is what makes `(bound - table) / 4
// = 4` a bound found in the CODE rather than an absence counted after the fact.
//
// **HOW THIS ARM DIFFERS FROM THE TWO ABOVE**, each difference a decoded byte:
//
//   * THE EMITTER IS INTERNAL STAGE 2's AND THE BRANCH IS INTERNAL STAGE 3's.
//     All four updaters end `4EF9 0023DEFC` (bucket 1, like internal stage 2,
//     UNLIKE internal stage 3's `$23DF2A`) and all four carry `6C00 bge.w` at
//     `upd+$0C` (like internal stage 3, UNLIKE internal stage 2's `6E00 bgt.w`).
//     The displacement after the branch is `$0006` in every one of the three
//     families, so `upd+$0C` is the only tell. That combination exists in
//     NEITHER neighbouring block, so a row copied from either would be silently
//     wrong in exactly one byte pair -- and BOTH are checked here.
//   * FOUR ENTRIES, FOUR DISTINCT STREAMS. W396's arm may not assert
//     `distinct === entries` because internal stage 3 contradicts it; on THIS
//     table it holds, and it is asserted as a MEASURED property rather than
//     inherited as a rule. There is no duplicate pair to allow for.
//   * EVERY `kind` IS THE BYTE FORM. `1D7C 0017 000D` in all four, so no row
//     here is `kindWord`. Internal stage 3's id 6 takes the word form
//     (`3D7C 0056 000C`, TRAP 3), and a table that had grown one would be
//     harvested by an arm that never looked -- so the form is checked.
//   * NOT ONE ENTRY IS COMPLEX. Internal stage 2's id 7 installs `$2629AE`,
//     which reverse-walks the 32-pair table `$262A4C`. All four rows here are
//     the plain `2D7C <data> 0010` shape with the `4E75` AT `ctor+$1C`, so this
//     arm harvests exactly one stream per row.
//
// `data` is a SPRITE-MASK offset and is resolved by `romExtent` against
// `sprmask`, never against `maincpu.bin`. `$31975C` is a long way from the
// other three and lands in the same chain regardless.
const STAGE5_BGELEM_TABLE = 0x2622f2;
/** `$262328 41FA FFD8 lea (-$28,PC),A0` -- the instruction that names the
 *  pointer array, and therefore the instruction that bounds the table. */
const BGELEM_PTRS_LEA = 0x262328;
{
  const handlers = BGELEM_HANDLERS.filter((h) => h.stage === 4);
  // THE BOUND, FOUND IN THE CODE. `lea (d16,PC),A0`: TRAP 4, the target is the
  // extension word's own address plus the displacement.
  if (romBe16(BGELEM_PTRS_LEA) !== 0x41fa) {
    throw new Error(`$${BGELEM_PTRS_LEA.toString(16)} is not \`41FA lea `
      + `(d16,PC),A0\` -- it reads ${romBe16(BGELEM_PTRS_LEA).toString(16)}. `
      + 'That instruction is the only statement in the cartridge of where the '
      + 'per-stage pointer array begins, and this table ENDS there.');
  }
  // **THE `lea` SUPPLIES THE ADDRESS; IT IS NOT COMPARED TO ONE.** Comparing
  // `leaTarget` against a constant typed into this file would put the bound
  // back where this arm is trying to take it out of. Everything below reads the
  // array THROUGH this target, so an instruction that names somewhere else
  // fails at the next guard by reading the wrong longword, by address.
  const ext = BGELEM_PTRS_LEA + 2;
  const leaTarget = ext + ((romBe16(ext) << 16) >> 16);
  const from = romBe32(leaTarget + 4 * 4);
  if (from !== STAGE5_BGELEM_TABLE) {
    throw new Error(`entry 4 of the per-stage handler-table array is $${
      from.toString(16)}; internal stage 4's table is $${
      STAGE5_BGELEM_TABLE.toString(16)}. The array was read at $${
      leaTarget.toString(16)}, which is where $${
      BGELEM_PTRS_LEA.toString(16)} \`41FA lea (d16,PC),A0\` points: extension `
      + `word $${ext.toString(16)} + $${romBe16(ext).toString(16)}. TRAP 4 -- `
      + 'the target is the EXTENSION WORD\'s address plus the displacement, and '
      + 'that instruction is the only statement in the cartridge of where the '
      + 'array begins.');
  }
  if ((leaTarget - from) / 4 !== handlers.length) {
    throw new Error(`internal stage 4's handler table is $${
      (leaTarget - from).toString(16)} bytes = ${(leaTarget - from) / 4
    } entries -- it runs from $${from.toString(16)} up to the pointer array at `
      + `$${leaTarget.toString(16)} -- and src/background.js carries ${
        handlers.length} \`stage: 4\` rows. This is the LAST entry of the array, `
      + 'so there is no next entry to bound the table with: the bound is the '
      + 'ARRAY\'S OWN BASE and the count is that subtraction, never a typed-in '
      + 'number. The cartridge decides how many elements the stage has; a short '
      + 'registry ships a subset of the terrain and a long one indexes the '
      + 'array itself.');
  }
  // ---------------------------------------------------------------- PASS ONE
  // THE TABLE, DECODED OUT OF THE IMAGE ALONE, exactly as W396's arm does it:
  // nothing here reads the registry, so these refusals stand even if
  // `src/background.js` is wrong in the way the pass below would otherwise hide.
  const romCtor = [], romData = [], romUpd = [], romYPos = [], romKind = [];
  for (let i = 0; i < handlers.length; i++) {
    const c = romBe32(STAGE5_BGELEM_TABLE + i * 4);
    romCtor.push(c);
    // $2631D4 `2D7C 003053A0 0010` -- move.l #imm,($10,A6). `0014` at +$6 would
    // be the Y constant (`3D7C` at +$8 writes that), a different field.
    if (romBe16(c) !== 0x2d7c || romBe16(c + 6) !== 0x0010) {
      throw new Error(`internal stage 4 background element ${i}: $${
        c.toString(16)} is not \`move.l #imm,($10,A6)\` -- it reads ${
        romBe16(c).toString(16)} ... ${romBe16(c + 6).toString(16)}`);
    }
    // $2631DC `3D7C 1EA0 0014` -- move.w #imm,($14,A6), the Y constant.
    if (romBe16(c + 8) !== 0x3d7c || romBe16(c + 0x0c) !== 0x0014) {
      throw new Error(`internal stage 4 background element ${i}: $${
        (c + 8).toString(16)} is not \`move.w #yPos,($14,A6)\` -- it reads ${
        romBe16(c + 8).toString(16)} ... ${romBe16(c + 0x0c).toString(16)}`);
    }
    // $2631E2 `2D7C 002631F2 0008` -- move.l #upd,($8,A6). The registry's `upd`
    // is read out of THIS rather than assumed from ctor+$1E, for the reason
    // W396 gave: internal stage 3's id 6 installs an updater that is not the
    // code following it, so adjacency is not the rule anywhere in this family.
    if (romBe16(c + 0x0e) !== 0x2d7c || romBe16(c + 0x14) !== 0x0008) {
      throw new Error(`internal stage 4 background element ${i}: $${
        (c + 0x0e).toString(16)} is not \`move.l #upd,($8,A6)\` -- it reads ${
        romBe16(c + 0x0e).toString(16)} ... ${romBe16(c + 0x14).toString(16)}`);
    }
    // $2631EA `1D7C 0017 000D` -- move.b #kind,($D,A6). THE BYTE FORM, all four.
    // Internal stage 3's id 6 is `3D7C 0056 000C`, a WORD over two byte fields
    // (TRAP 3); a row of that shape here would carry a `kind` this arm read as
    // a byte and `elemStage` reads as a word.
    if (romBe16(c + 0x16) !== 0x1d7c || romBe16(c + 0x1a) !== 0x000d) {
      throw new Error(`internal stage 4 background element ${i}: $${
        (c + 0x16).toString(16)} is not \`move.b #kind,($D,A6)\` -- it reads ${
        romBe16(c + 0x16).toString(16)} ... ${romBe16(c + 0x1a).toString(16)}. `
        + 'All four entries on this table take the BYTE form; internal stage '
        + '3\'s id 6 takes the word form $3D7C/$000C and covers TWO byte fields.');
    }
    // TRAP 5: the `4E75` sits AT ctor+$1C, so the unit is $1E bytes.
    if (romBe16(c + 0x1c) !== 0x4e75) {
      throw new Error(`internal stage 4 background element ${i}: $${
        c.toString(16)} does not close with \`4E75 rts\` AT ctor+$1C -- it `
        + `reads ${romBe16(c + 0x1c).toString(16)}. The five-instruction `
    ...
    romData.push(romBe32(c + 2));
    romYPos.push(romBe16(c + 0x0a));
    romUpd.push(romBe32(c + 0x10));
    romKind.push(romBe16(c + 0x18));
  }
  // FOUR ENTRIES, FOUR DISTINCT DESCRIPTORS. `elemConstruct` writes ($10,A6)
  // ONCE and nothing else ever writes it, so a duplicate would be one element
  // drawn with two `kind` bytes -- which is what internal stage 3 IS, and what
  // this table is NOT. Stated as a number, so a table that grew a duplicate is
  // a refusal rather than a quietly shorter harvest.
  const distinctData = new Set(romData).size;
  if (distinctData !== handlers.length) {
    throw new Error(`internal stage 4's table $${
      STAGE5_BGELEM_TABLE.toString(16)} has ${handlers.length} entries and ${
      distinctData} distinct descriptors (${
      romData.map((d) => '$' + d.toString(16)).join(' ')
    }); the cartridge has NO duplicate pair here. Internal stage 3 does have `
      + 'one -- $2B01D0, its entries 0 and 6 -- and the arm above allows for it '
      + 'by measurement; allowing for one here would harvest a table this file '
      + 'has not read.');
  }
  if (new Set(romUpd).size !== handlers.length) {
    throw new Error(`internal stage 4's ${handlers.length} entries install only `
      + `${new Set(romUpd).size} distinct updaters (${
        romUpd.map((u) => '$' + u.toString(16)).join(' ')
      }); on this table every entry has its own, so a shared one means two `
      + 'elements are running one despawn test and `BGELEM_BY_UPD` collides.');
  }
  ...
  for (let i = 0; i < handlers.length; i++) {
    const h = handlers[i];
    if (h.id !== i) {
      throw new Error(`internal stage 4 row ${i} carries id ${h.id}; these rows `
        + 'are indexed by op $10\'s id through this very table');
    }
    if (romCtor[i] !== h.ctor) {
      throw new Error(`internal stage 4 background element ${i}: the `
        + `cartridge's own table $${STAGE5_BGELEM_TABLE.toString(16)} names $${
          romCtor[i].toString(16)} and src/background.js's row ${i} is $${
          h.ctor.toString(16)}`);
    }
    if (romData[i] !== h.data) {
    ...
    if (romYPos[i] !== h.yPos || romKind[i] !== h.kind || h.kindWord) {
      throw new Error(`internal stage 4 background element ${i} ($${
        h.ctor.toString(16)}): the cartridge writes yPos $${
        romYPos[i].toString(16)} / kind $${romKind[i].toString(16)} and `
        + `src/background.js says $${h.yPos.toString(16)} / $${
          h.kind.toString(16)}${h.kindWord ? ' (kindWord)' : ''}. Both ride the `
        + 'sprite record `elemStage` writes, so a wrong one draws the right '
        + 'picture in the wrong place or the wrong colour.');
    }
    // $2631F8 `0680 00003C00` -- addi.l #thr,D0, the despawn threshold, read
    // out of the instruction that applies it.
    if (romBe16(h.upd + 6) !== 0x0680 || romBe32(h.upd + 8) !== h.thr) {
      throw new Error(`internal stage 4 background element ${i}: updater $${
        h.upd.toString(16)} does not carry \`0680 addi.l #$${
        (h.thr >>> 0).toString(16)},D0\` at upd+$6 -- it reads ${
        romBe16(h.upd + 6).toString(16)} $${
        romBe32(h.upd + 8).toString(16)}. That immediate IS the registry's `
        + '`thr` and it decides the frame the element despawns on.');
    }
    // $2631FE `6C00 0006` -- bge.w. TRAP 19 and TRAP 4: internal stage 2's
    // fourteen carry `6E00 bgt.w` HERE with the SAME $0006 displacement, so
    // this byte pair is the whole difference between `lbge` and `lbgt` and a
    // one-frame difference at the despawn edge.
    if (romBe16(h.upd + 0x0c) !== 0x6c00 || romBe16(h.upd + 0x0e) !== 0x0006
        || h.v !== 'lbge') {
      throw new Error(`internal stage 4 background element ${i}: updater $${
        h.upd.toString(16)} does not carry \`6C00 bge.w $0006\` at upd+$C -- it `
        + `reads ${romBe16(h.upd + 0x0c).toString(16)} ${
          romBe16(h.upd + 0x0e).toString(16)} against registry variant '${h.v
        }'. \`6E00\` is bgt.w and internal stage 2's fourteen carry exactly `
        + 'that with the same displacement, so a row copied from there kills '
        + 'this element one frame early on a true sum of exactly 0.');
    }
    // $263222 `4EF9 0023DEFC` -- the updater's tail jump. BUCKET 1, like
    // internal stage 2 and UNLIKE internal stage 3's $23DF2A. The other half of
    // the pair of constants that flipped between adjacent tables.
    if (romBe16(h.upd + 0x2c) !== 0x4ef9 || romBe32(h.upd + 0x2e) !== h.emit
        || h.emit !== 0x23defc) {
      throw new Error(`internal stage 4 background element ${i}: updater $${
        h.upd.toString(16)} does not end \`4EF9 $23DEFC\` -- it reads ${
        romBe16(h.upd + 0x2c).toString(16)} $${
        romBe32(h.upd + 0x2e).toString(16)} against registry $${
        (h.emit >>> 0).toString(16)}. A row aliased onto internal stage 3's `
        + 'updater would put this stage\'s terrain in the wrong sprite bucket.');
    }
    seen.add(h.data);
    if (streams.has(h.data)) { already++; continue; }
  ...
  harvested += added; harvestAlready += already;
  harvestReport.push({ shard: STRUCT_SHARD, base: STAGE5_BGELEM_TABLE,
    entries: handlers.length, stride: 4, runsTo: handlers.length,
    endsAt: leaTarget,
    distinct: seen.size, added, already,
    why: 'W397 INTERNAL STAGE 4 (human Stage 5): all 4 elements\' own sprites, '
      + 'four distinct streams, extent taken from the per-stage pointer array '
      + '$262302 (entry 4 $2622F2, bounded by the ARRAY\'S OWN BASE -- there is '
      + 'no entry 5 -- which $262328 `41FA FFD8 lea (-$28,PC),A0` states) and '
      + 'each descriptor read out of its own `move.l #imm,($10,A6)`. THE LAST '
      + 'BGELEM TABLE: 46 rows over 5 tables, all harvested' });
  console.log(`  internal stage-4 background elements: ${handlers.length} `
    + `handlers, ${seen.size} distinct streams, ${added} new -- STAGE 5's `
    + 'TERRAIN, and the LAST BGELEM table');
}
...
// bomb, and NEITHER HAS A PICTURE: W64 Ã‚Â§8.3 counted 174 bucket-13 records with
// no shard behind them and W65 Ã‚Â§7.3 named three missing streams off the page's
...
// and W58 Ã‚Â§2.2's four families are what a measured floor costs).
//
// **NONE OF IT IS IN OR BEHIND E3's HOLE.**  W58 Ã‚Â§7.1 left `$24B900..$24BB0A`
 ...
 *  E3 Ã‚Â§2.1(b)'s mechanism, and the same completeness argument: the port cannot
 ...
 *  emitting a record at all (Ã‚Â§4).  A fixture that sits where two readings agree
...
// from `20-recon-level-data.md` Ã‚Â§0/Ã‚Â§1/Ã‚Â§3b; every one of the checks below fails
...
// CHECK 2 -- the counts and the ranges the recon measured.
{
  ...
  if (mapTiles.size !== 1820 || lo !== 0x0aa9 || hi !== 0x11c6) {
    throw new Error(`stage 1's 224 columns hold ${mapTiles.size} distinct tiles `
      + `$${lo.toString(16)}..$${hi.toString(16)}; recon Ã‚Â§1/Ã‚Â§2 measured 1,820 `
  ...
  if (smapTiles.size !== 205 || slo !== 0x32a9 || shi !== 0x3381) {
    throw new Error(`the second map holds ${smapTiles.size} distinct tiles `
      + `$${slo.toString(16)}..$${shi.toString(16)}; recon Ã‚Â§3b measured 205 in `
      + '$32A9..$3381.');
  }
...
// and a byte-swapped read scatters them.  Recon Ã‚Â§1a measured 0 of 1024.
...
for (let i = 0; i < STAGE1.palWords; i++) bgPal[i] = be16(STAGE1.pal + i * 2);
{
  ...
  if (set !== 0) {
    throw new Error(`${set} of ${STAGE1.palWords} words at $227E58 have bit 15 `
      + 'set. xRGB555 never does; recon Ã‚Â§1a measured 0. This is not the palette '
...
const manifest = {
  ...
  game: gameId, set: revision.set, build: revision.build,
  ...
  gfx: {
    ...
    bg: {
      ...
      secondMap: {
        ...
        //    What spawns type $1C is named-not-found (recon Ã‚Â§8.5)."
  ...
  spr: {
    ...
    // translated (40-recon-emission-path.md Ã‚Â§4 step 2, measured).
    //
    ...
    // indented line per stream, not a comma and a number.  (43-plan Ã‚Â§3.1(a)
...
//
// W47 Ã‚Â§2.4 established the rule this is an application of: `manifest.json` is
...
// or number is dropped -- the prose W47 Ã‚Â§2.3 needs for "SPRITE SHARD 1 DID NOT
