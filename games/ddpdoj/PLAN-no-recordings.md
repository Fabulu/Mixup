...
| L4 | Player shot sprites (worse than replayed: NOT in the capture at all — the port computes shots and they are INVISIBLE) | W13 | **CLOSED (W52). THE SHOTS ARE DRAWN.** They were never in the capture, so W37's strip had nothing to remove and W44's remap had nothing to point at; what was missing was the ART. W52 harvested it BY ADDRESS from the four 38-byte-template tables the port itself reaches -- the ship's `$2554EA`/`$255502` and the option pods' `$24D2FC`/`$24D35C`, five power steps each, three chains per template -- **71 streams / 10.5 KiB gz into DEFERRED sprite shard 6**. [M] Over 1,200 logic frames from the shipped seed with fire tapped every 4 frames, bucket 14 carries 21,691 records over 20 distinct streams and **0 of 20 had art before, 20 of 20 draw now**. Recon 40's "nine streams, 2,184 bytes" was a floor: it measured under `--no-pods`, and L3 unblocked the pods, which are a SECOND shot producer. Still open on this row: the LASER's own shot templates (`$2554EA[+4]`, type word `$8004` = dispatch entry [4] = `$254078`, unported and throwing) are deliberately NOT harvested, and the exporter asserts they stay unported |
...
| L10 | The enemies: existence, position, motion, FACING and AIM (aim reads the live player 3.5×/frame — 10-recon-enemies §5 — a recording cannot supply it, ever) | W18–W20, W22, W26 | **ON SCREEN, from the port's own display list (W44).** W37 took the RECORDED enemies off; W44 put the PORT'S on. The port spawns, moves, aims and damages them and they are now DRAWN: [M] bucket 0 carries 14 to 62 records a frame from the page's own seed, 100 % of them with art for the first 5.32 s. **AND W47 GAVE THEM BODIES.** The owner reported turrets driving around with no tank underneath; [M] type $11's HULL table `$268B9E` is 64 images by HEADING and the shipped sheet held TWO of them, because the recording the sheet was harvested from drove every tank one way (`46-diag-orphan-turrets.md`). W47 harvested seven sprite-pointer tables BY ROM ADDRESS -- 212 streams, 209 KiB -- into five DEFERRED sprite shards, and [M] over the same 6,185-frame run the hull table went **74,826 emitted / 19,252 drawn -> 74,826 / 74,826**, missed records **154,831 -> 85,897 (-44.5 %)** and distinct missing addresses **326 -> 192**, with zero streams of any harvested table left in the miss set. Still open on this row: the eight unported stage-1 handlers (the run still stops at the named `$292902` throw after 6,185 frames) and the 192 remaining cartridge stream addresses, which belong to OTHER producers -- the biggest is `$12D430` at 14,104 records |
| L11 | Enemy bullets (buckets 22/23, the front of the picture) | W23 | **CLOSED for bucket 23 (W52). THE BULLETS ARE DRAWN.** The recorded ones went in W37; the ported ones did not draw because `src/mover.js spriteEmit` wrote into a JS array and `bulletdriver.js` passed none -- [M] buckets 22 and 23 measured **0 records over 1,200 frames** with and without fire, while 14,172 live bullet record-frames went past. W52 made A4 a real cursor into `$809C4C`, ported `$283194`'s trail block (a MOVE, not a copy -- A4 rewinds permanently), and harvested the art by walking the mask ROM's own stream chain across four ranges: **306 streams / 15.5 KiB gz into DEFERRED sprite shard 7**, each range closing EXACTLY on its stated end or the build stops. [M] bucket 23 now carries 2,432 records over 300 no-input steps and 7,465 over 2,204 tapped ones, with **0 missing art**. It also fixed `26-review` F1 and F2 and a THIRD of the same family the sink found (`$282748`, kind 7's ring, 2,478 records over 66 addresses that are not stream starts). **Still open: BUCKET 22.** [M] It is 0 in every run this project has made -- the trail block is kinds 27/36/37/38's and none of them spawns in stage 1's opening. Ported and unit-tested; unreached. Presence, not absence |
...
| L13 | The laser beam, bomb flash, hyper (NOT in the capture at any price — the recorded player never held fire or bombed) | W24, W25 | open -- **but the beam's ART now ships (W47).** W45 built the beam and measured that not one of its five streams (`$01302C` `$013098` `$065354` `$011E8C` `$013B94`) was in the 166-stream sheet, so every beam record was a named skip. They are harvested by address into sprite shard 1, the FIRST deferred fetch, for 1.1 KiB. Not marked REPLACED: nothing in this wave has SEEN a beam drawn, and the bomb and hyper are untouched |
| L14 | The palette DURING gameplay: per-stage loads are located (`$2415E8`, W16); per-frame writers/fades were not reconned | **not yet answered** — needs a palette-writer census before W17 closes. **W44 answers it for the SPRITE banks over the recorded window: [M] the port's records ask for 16 of the 32 sprite colour banks and the page draws all of them against the capture's palette; bank 24 (the three ground shadows) is all zeros on both sides.** The BG half of the row is unchanged. | open (BG half) |
