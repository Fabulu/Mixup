// W633: export the complete Stage-1 boss E8 carrier art family.
...
const TABLE_START = 0x296f68;
const TABLE_END = 0x296fa8;
const STREAM_COUNT = 16;
const REQUEST_SIZE = 0x0618;
const MASK_WORDS = 74;
const required = [
  IMAGE,
...
function tablePointers() {
  assert.equal(TABLE_START + STREAM_COUNT * 4, TABLE_END,
    'the sixteen-entry table closes exactly at $296FA8');
  return Array.from({ length: STREAM_COUNT }, (_, index) =>
    IMG.readUInt32BE(TABLE_START + index * 4) & 0x7fffff);
}

test('W633 all sixteen carrier streams retain exact packaged cartridge geometry',
  { skip: SKIP }, async () => {
    const requested = tablePointers();
    assert.deepEqual([requested.length, new Set(requested).size], [16, 16]);
    assert.equal(requested[0], 0x084340);
    ...
    for (const offset of requested) {
      ...
      assert.equal(extent.maskWords, MASK_WORDS,
        `$${offset.toString(16)} has the complete 3 by 24 mask extent`);
...
test('W633 the cold-campaign $084340 request reaches the packed draw path',
  { skip: SKIP }, async () => {
    const b = await bundle();
    const packed = romToPackedMap(b.manifest, b.spr.shardOfBase.bind(b.spr));
    const ram = new Ram();
    const offset = tablePointers()[0];
