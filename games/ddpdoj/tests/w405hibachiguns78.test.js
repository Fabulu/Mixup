...
test('W405 SECTION 3: what ends the loop is A4 $D\'s own ($1A,A5) byte, and the stop is A4 $F',
  { skip: SKIP }, () => {
    ...
    assert.deepEqual(r.a4.slice(-4).map(([, v]) => v), ['b', '3', '4', 'f'],
    ...
    // ---- **WHICH KIND OF STOP.** Three tests, none of them an absence.
    assert.deepEqual(r.stopped, { frame: 2928, at: 0x2a6a30, name: 'Unreached' },
      'the run now stops on frame 2928 at $2A6A30 -- 1,946 frames past W404\'s 982');
    //   (a) it is a live table entry the cartridge dispatches through;
    assert.equal(l(HIBACHI_A4.table + 0x0f * 8), 0x2a6a30,
      '(a) $2A5886[$F].init IS $2A6A30, reached by $2596FA jsr (A0)');
    assert.equal(w(0x2a6a30), 0x397c,
      '  ...and `397C` move.w #imm,(d16,A4) stands there: ordinary code, not an rts, not a park');
    //   (b) something the cartridge wrote is what routed us there -- and it is NOT the loop;
    assert.equal(HIBACHI_END_COUNTED[0x0f].init, 0x2a6a30,
      '(b) hibachiend.js has counted it since W403, at $46 bytes, as script 4\'s hand-over');
    //   (c) the code at that address has somewhere to go: it waits on A1 gun 9.
    assert.equal(w(0x2a6a4a), 0x7009, '(c) $2A6A4A moveq #$9,D0');
    assert.equal(l(0x2a6a4e), 0x00259a18, '  ...$2A6A4C jsr $259A18 -- A4 $F starts A1 GUN 9');
    assert.equal(HIBACHI_A1_COUNTED[9].init, 0x2a89ba, '  ...which is counted here at $1C2 bytes');
    // ...and it is a PORT stop and not a data stop: neither the script nor the gun is registered.
    const reg = new Set(scriptAddresses());
    assert.ok(!reg.has(0x2a6a30), 'A4 $F is not registered by any file');
    assert.ok(!reg.has(0x2a89ba), '  ...and neither is gun 9');
  });

test('W405 SECTION 3: 3,745 bullets, and every count is somebody\'s volley arithmetic',
  { skip: SKIP }, () => {
    const r = runReal(realPath(), 8000);
    assert.equal(r.shots, 3745, 'the run fires 3,745 where W404\'s fired 1,260');
...
test('W405 SECTION 7: three new windows, 593, each sized by its own instructions', { skip: SKIP },
  () => {
    ...
    assert.equal(tables.rom.windows.length, 593, '593 windows, 590 + this wave\'s three');
...
test('W405 SECTION 7: guns 7 and 8 are PORTED, and the counted list shrank by exactly two',
  { skip: SKIP }, () => {
    assert.deepEqual([...HIBACHI_A1_SCRIPTS], [5, 6, 7, 8], 'four A1 ids are ported now');
    assert.deepEqual([...HIBACHI_GUN_A4_SCRIPTS], [0x0a, 0x0b, 0x0c, 0x0d], '  ...and four A4');
    for (const id of [7, 8]) {
