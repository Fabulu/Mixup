...
const SOURCE = 0x131464;
const MOVEMENT = 0x131a16;
const REC = ENEMY.bandCommon;
const SUB = 0x81459c;
const QUEUE_7_BYTES = 0x80afc8;
const QUEUE_3_BYTES = 0x80afc6;
const TYPE08_WINDOWS = Object.freeze([
  Object.freeze({ base: '$1668DC', len: 0x0008 }),
  Object.freeze({ base: '$16952C', len: 0x0008 }),
  Object.freeze({ base: '$16962A', len: 0x0016 }),
  Object.freeze({ base: '$169640', len: 0x001c }),
]);
...
function assertWhiteOnly(reads) {
  ...
  'every White Type $08 cartridge read stays below $200000');
}
...
function spawnWhite08() {
  ...
  ram.setU16(world.resources.spawn.distanceClock, 0x0178);
...
test('White Type $08 owns its descriptors, four windows, and natural $131464 route', () => {
  const black = BLACK_WORLD_RESOURCES.enemyTypes[0x08];
  const white = WHITE_WORLD_RESOURCES.enemyTypes[0x08];
  ...
  assert.deepEqual([
    ...
    0x26a4b4, 0x26a4bc, 0x26a5e4,
    0x26a5b2, 0x26a5c8,
    0x269bb6, 0x269e48, 0x269ec8, 0x269f48,
    0x23d852, 0x23df86, 0x23df58,
    0x26a618, 0x242a80, 0x24202c,
    0x2814ac, 0x26a782, 0x28c2a8, 0x263762,
  ]);
  assert.deepEqual([
    ...
    0x16952c, 0x169534, 0x16965c,
    0x16962a, 0x169640,
    0x168c2e, 0x168ec0, 0x168f40, 0x168fc0,
    0x13dba0, 0x13e2d4, 0x13e2a6,
    0x169690, 0x142dd0, 0x142366,
    0x1804f8, 0x1697fa, 0x18adce, 0x1627dc,
  ]);
  ...
  assert.throws(() => requireType08Resources({ ...white }, 'white'),
    /canonical frozen edition descriptor/);
  assert.throws(() => requireType08Resources(black, 'white'),
    /canonical frozen edition descriptor/);
  ...
  assert.deepEqual(tables.editions.whiteLabel.stage1Type08, {
    init: {
  ...
  assert.equal(TYPE08_WINDOWS.reduce((sum, window) => sum + window.len, 0), 0x42);
  const manifest = tables.editions.whiteLabel.worldRuntimeWindows;
  for (const expected of TYPE08_WINDOWS) {
  ...
  const { ram, rom, reads, machineCtx, world } = spawnWhite08();
  assert.deepEqual(Array.from(rom.bytes(SOURCE, 8)), [
    0x01, 0x78, 0x00, 0x08, 0x08, 0x00, 0x00, 0x23,
  ]);
  ...
  assert.deepEqual([rom.u32(0x1668dc), rom.u32(0x1668e0)],
  ...
  for (const address of [
    SOURCE, 0x131752, MOVEMENT, 0x1668dc,
...
test('White Type $08 transitions, emits both native draw arms, and fires a continuing kind $0D bullet', () => {
  const transition = spawnWhite08();
  const descriptor = transition.world.resources.enemyTypes[0x08];
  isolateHandler(transition, { speed: 1, mirror: 1 });
  ...
  assert.deepEqual([
    transition.ram.u16(REC + 0x26), transition.ram.u8(SUB + 0x1a),
    transition.ram.u8(REC + 0x18), transition.ram.u8(SUB + 0x1b),
  ], [1, 0, 0x0f, 0x10]);
  ...
  const firing = spawnWhite08();
  isolateHandler(firing, { speed: 0, mirror: 0 });
  ...
  runHandler(descriptor.handler, firing.ram, firing.rom, REC, ctx, firing.world.resources);
  ...
  assert.equal(firing.ram.u8(bullet + BULLET_REC.speed), 0x17);
...
test('White Type $08 lethal P2 damage credits only P2 and uses native effect, sound, and retirement', () => {
  const fixture = spawnWhite08();
  const { ram, rom, reads, world } = fixture;
  const descriptor = world.resources.enemyTypes[0x08];
