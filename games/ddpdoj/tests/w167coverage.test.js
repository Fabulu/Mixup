...
test('W202 config is a machine-readable family map with an exact backlog', () => {
  ...
  assert.equal(stage3.derived_ported_type_count, 26);
...
test('W202 reusable coverage derives the current closed-family totals', { skip: !evidence }, () => {
  const got = run();
  ...
  assert.match(got.stdout, /enemy_types: 61\/256 ported, 65 unknown, 130 null/);
  assert.match(got.stdout, /stage1_spawn_script: 339\/339 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 332\/332 ported/);
  assert.match(got.stdout, /stage3_spawn_script: 375\/414 ported, 39 unknown, 0 null/);
  assert.match(got.stdout, /stage3_enemy_frontier: 39 ordered records/);
  assert.match(got.stdout, /stage3_enemy_types: 26\/28 covered types/);
  const romBytes = readFileSync(rom);
  assert.equal(romBytes[0x234C92 + 4], 0x16,
    'next unsupported Stage-3 record is type $16 at $234C92');
...
test('W202 both regression conditions demonstrably go red', { skip: !evidence }, () => {
