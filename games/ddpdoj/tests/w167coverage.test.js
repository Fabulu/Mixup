...
test('W167 config is a machine-readable family map with an exact backlog', () => {
  ...
  assert.equal(frontier.remaining_records.length, 2);
  assert.deepEqual(frontier.remaining_records[0].slice(0, 5),
    [0x233018, 0x01d5, 0x86, 0x275bb6, 0x275914]);
});
...
test('W167 reusable coverage derives the current closed-family totals', { skip: !evidence }, () => {
  ...
  assert.match(got.stdout, /enemy_types: 41\/256 ported, 85 unknown, 130 null/);
  assert.match(got.stdout, /stage1_spawn_script: 339\/339 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 330\/332 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 330\/332 ported, 2 unknown, 0 null/);
