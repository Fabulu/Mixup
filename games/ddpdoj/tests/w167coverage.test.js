...
test('W167 config is a machine-readable family map with an exact backlog', () => {
  ...
  assert.equal(frontier.remaining_records.length, 3);
  assert.deepEqual(frontier.remaining_records[0].slice(0, 5),
    [0x232ef0, 0x0197, 0x93, 0x279ec2, 0x279f4a]);
});
...
test('W167 reusable coverage derives the current closed-family totals', { skip: !evidence }, () => {
  ...
  assert.match(got.stdout, /enemy_types: 40\/256 ported, 86 unknown, 130 null/);
  assert.match(got.stdout, /stage1_spawn_script: 339\/339 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 329\/332 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 329\/332 ported, 3 unknown, 0 null/);
