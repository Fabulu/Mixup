...
test('W198 config is a machine-readable family map with an exact backlog', () => {
  ...
  assert.equal(stage3.derived_ported_type_count, 23);
...
test('W199 reusable coverage derives the current closed-family totals', { skip: !evidence }, () => {
  const got = run();
  ...
  assert.match(got.stdout, /enemy_types: 56\/256 ported, 70 unknown, 130 null/);
  assert.match(got.stdout, /stage1_spawn_script: 339\/339 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 332\/332 ported/);
  assert.match(got.stdout, /stage3_spawn_script: 358\/414 ported, 56 unknown, 0 null/);
  assert.match(got.stdout, /stage3_enemy_frontier: 56 ordered records/);
  assert.match(got.stdout, /stage3_enemy_types: 23\/28 covered types/);
  const romBytes = readFileSync(rom);
  assert.equal(romBytes[0x234AF2 + 4], 0x15,
    'next unsupported Stage-3 record is type $15 at $234AF2');
