...
test('W203 config is a machine-readable family map with an exact backlog', () => {
  ...
  assert.equal(stage3.derived_ported_type_count, 27);
...
test('W203 reusable coverage derives the current closed-family totals', { skip: !evidence }, () => {
  const got = run();
  ...
  assert.match(got.stdout, /enemy_types: 62\/256 ported, 64 unknown, 130 null/);
  assert.match(got.stdout, /stage1_spawn_script: 339\/339 ported/);
  assert.match(got.stdout, /stage2_spawn_script: 332\/332 ported/);
  assert.match(got.stdout, /stage3_spawn_script: 413\/414 ported, 1 unknown, 0 null/);
  assert.match(got.stdout, /stage3_enemy_frontier: 1 ordered records/);
  assert.match(got.stdout, /stage3_enemy_types: 27\/28 covered types/);
  const romBytes = readFileSync(rom);
  assert.equal(romBytes[0x234FA2 + 4], 0xA0,
    'next unsupported Stage-3 record is type $A0 at $234FA2');
...
test('W203 both regression conditions demonstrably go red', { skip: !evidence }, () => {
