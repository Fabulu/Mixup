...
test('the Stage-1 through W223 Stage-4 bodies are dispatched', () => {
  ...
  assert.ok(INIT_BODY_ADDRESSES.includes(0x2671e8),
    'W201: Stage-3 pulse-controller body $2671E8');
  ...
  assert.ok(INIT_BODY_ADDRESSES.includes(0x29e580),
    'W209: Stage-3 boss low-HP child type $99 body $29E580');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27896a),
    'W211: Stage-4 opening pulse-controller body $27896A');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27ac4a),
    'W212: Stage-4 linked-structure body $27AC4A');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27cfac),
    'W213: Stage-4 opening/rotating gun-pod body $27CFAC');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27ad96),
    'W214: Stage-4 root-ship/satellite-array body $27AD96');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27b2fe),
    'W215: Stage-4 three-part carrier body $27B2FE');
  ...
  // W229 catches the census up with the four bodies the Stage-4 waves added
  // after W217 without updating this count.
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27c5be),
    'W218: Stage-4 body $27C5BE');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27da78),
    'W218: Stage-4 body $27DA78');
  ...
  assert.ok(INIT_BODY_ADDRESSES.includes(0x2659e4),
    'W317: type $59 body $2659E4 -- one prototype and one two-byte-field word');
  assert.ok(INIT_BODY_ADDRESSES.includes(0x27640c),
  ...
  assert.ok(INIT_BODY_ADDRESSES.includes(0x273f0e),
    'W326: the REAL type $81 body $273F0E -- two spawn-time aims and a per-stage row');
  ...
  // W484: 91 -> 92, type $50's body $270402, emitted by type $4C's part-4 animator.
  assert.ok(INIT_BODY_ADDRESSES.includes(0x270402),
    'W484: type $50 body $270402 -- type $4C\'s part-4 runtime child and a source for type $51');
  // W485: 92 -> 93, type $51's body $2704D0, emitted when type $50 expires.
  assert.ok(INIT_BODY_ADDRESSES.includes(0x2704d0),
    'W485: type $51 body $2704D0 -- type $50\'s terminal runtime child');
  ...
  // W529: 95 -> 96, type $A5's body $2783F6, reached by pair {0,4} in stage 3.
  assert.ok(INIT_BODY_ADDRESSES.includes(0x2783f6),
    'W529: type $A5 body $2783F6 -- the deferred six-pair progression blocker');
  assert.equal(INIT_BODY_ADDRESSES.length, 96,
    ...
    + `W316's $270DD8, W317's $2659E4, W319's $27640C, W323's $26925E, W400's $26DF48, W481's $27063C, W482's $2701DE, W483's $2702A0, W484's $270402, W485's $2704D0, W487's $270BE4, W522's $267262, and W529's $2783F6`);
});
