...
test('W403 SECTION 1: exactly TWO instructions in 6 MB write ($10E,A6), and they write 1 and 2',
  { skip: SKIP }, () => {
    ...
    assert.deepEqual(HIBACHI_END_SCRIPTS.slice().sort((a, b) => a - b), [0, 1, 2, 3, 4, 5, 6, 7, 0x14],
      '  ...and scripts 0 through 7 plus $14 are registered now, including W562 script 7');
...
test('W403 SECTION 5: phase A dies of DAMAGE and the whole ending chain runs to A4 $F',
  { skip: SKIP }, () => {
    ...
    // together they are 389 frames of real work, not a throw. W407: $11, gun $B and $10 add
    // another 699, so the budget doubles again.
    const r = drive(b, 2400);
...
test('W403 SECTION 7: the window set, an overlap count this wave does not move, and '
  + 'NOTHING in the unit reads ROM',
  { skip: SKIP }, () => {
    const ws = WINDOWS();
    assert.equal(ws.length, ROM_WINDOW_COUNT, '585 windows through W403, which declared none; W404 added '
