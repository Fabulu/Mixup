...
//   * W180 closes all six type $94 records; the walker continues to the next honest
//     stop: type $93 body `$279EC2`, record `$232EF0`, clock `$0197`.
//
...
const SEED_LF = 19500;
const MAX_FRAMES = 7000;               // crosses clock $197 and reaches W180's next stop
...
test('W133/3 booting from lf19500 reaches stage 2 and stops honestly at type '
  + '$93 init body after W180', { skip: SKIP }, () => {
  const r = bootStage2();
  ...
  assert.strictEqual(r.threw.romAddress, 0x279ec2);
  assert.strictEqual(r.throwClock, 0x0197);
...
test('W133/4 installer replaces the old terminator and the exact ordered prefix '
  + 'completes before type $93', { skip: SKIP }, () => {
  const r = bootStage2();
  ...
  assert.strictEqual(r.game.allocEvents.get('spawn-script'), 288,
    '288 of the 292 consumed records allocate slots; four authentically decline');
  assert.strictEqual(r.game.ram.u32(LIVE_CURSOR), 0x232ef0,
    'the live cursor names the exact record whose init callback throws');
  assert.strictEqual((r.game.ram.u32(LIVE_CURSOR) - 0x2325d0) / 8, 292,
    'the live cursor proves the 292-record prefix completed before type $93');
});
