...
test('W556 object 2 is the live next blocker after object 1 emits', { skip: SKIP }, () => {
  const b = bench();
  a2Run2598E6(b.ram, 1);
  a2Run2598E6(b.ram, 2);
...
test('W556 ships the one fixed lower-body stream in the boss shard',
  { skip: SKIP_ASSETS }, () => {
    ...
    assert.deepEqual(row, { base: 2234580, maskWords: 4610 });
    ...
    assert.equal(shard.streams, 1238);
    assert.equal(shard.maskLen, 779972);
    assert.equal(shard.colLen, 1968237);
