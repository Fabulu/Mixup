...
test('a hold AT THE SPEED FLOOR still throws -- the wave-9 gate could not', () => {
  const g = bench();
  seedOptions(g.ram);
  // The floor: ($1a,A4) == ($38,A4) == 12, exactly the state the old guard's
  // `speedIdx !== laserFloor` declared uninteresting and skipped.
  g.ram.setU8(RAM.player1 + P.speedIdx, 12);
  g.ram.setU8(RAM.player1 + P.laserFloor, 12);
  g.ram.setU8(RAM.p1Options + OPT.raw, 0x10);
  g.ram.setU8(RAM.player1 + P.dirByte, 0x10);      // $24C134 copies THIS
  const e = caught(() => runOptionObject(g.ram, g.ctx));
  assert.ok(e instanceof Unreached);
  assert.equal(e.romAddress, ROM.optionLaser,
    'the throw names $24C180, the laser branch -- not $24C8BE, the ramp');
  assert.match(e.message, /HOLDING Button 1/);
});

test('ONE held frame is enough: the board gate is not a counter', () => {
  const g = bench();
  seedOptions(g.ram);
  g.ram.setU8(RAM.player1 + P.dirByte, 0x10);      // first held frame
