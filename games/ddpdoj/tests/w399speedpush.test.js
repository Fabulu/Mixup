...
test('W399 SECTION 2: nine callers of $261100, FOUR of them unclaimed and two pushing $0020',
  { skip: SKIP }, () => {
    ...
    assert.deepEqual(HIBACHI_END_SCRIPTS.slice().sort((a, b) => a - b), [0, 1, 2, 3, 4, 5, 6, 7, 8, 0x14],
      'A4 0 from W552, 1, 2 and 3 from this wave, 4 from W403, 5 from W409, 6 from W561, 7 from W562, 8 from W563 and $14');
...
test('W399 SECTION 7: the window set, an overlap count these five do not move, and all five '
  ...
  { skip: SKIP }, () => {
    ...
    // The nearest declared neighbours, both sides. W554 later placed Hibachi's A3
    // scheduler table below this wave, ending at $2A54D2.
    const others = ws.filter(([a]) => !mine.includes(a));
    const below = Math.max(...others.filter(([a, ln]) => a + ln <= HIBACHI_A4.table)
      .map(([a, ln]) => a + ln));
    assert.equal(below, 0x2a54d2,
      'the nearest window below is W554\'s A3 scheduler table ending at $2A54D2');
