...
test('SECTION 2: `$246520` and `$24652A` are each claimed EXACTLY ONCE, and by the same file', () => {
  ...
  // see private functions, arrows and methods reports 73 after current resource seams remove
  // $24C096 and add $27CBB6 and $289FC0. SECTION 2d below and
  ...
  assert.equal(dup.length, 20,
    ...
    + 'wrappers add $27CBB6, $289F96 and $289FC0, so the live narrow floor is 20. An '
    + 'unclassified duplicate is not absorbed here: ' + dup.map((a) => '$' + a.toString(16).toUpperCase()).join(', '));
});

test('SECTION 2d [W450/current]: the widened register is 73, and this wave\'s three-copy constructor '
