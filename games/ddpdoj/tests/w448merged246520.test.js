...
test('SECTION 2: `$246520` and `$24652A` are each claimed EXACTLY ONCE, and by the same file', () => {
  ...
  // see private functions, arrows and methods reports 72 after W614 registers the documented
  // $249D2C facade/resource split. SECTION 2d below and tests/w450widenedregister.test.js
  // SECTION 3 hold the set.
  const dup = [...idx].filter(([, v]) => v.size > 1).map(([a]) => a).sort((x, y) => x - y);
  assert.equal(dup.length, 17,
    'W474 left 15; W497 registers the authentic $2491C0 seed adapter, and W614 registers '
    + 'the separately named $2497AA shot adapters, so the live narrow floor is 17. An '
    + 'unclassified duplicate is not absorbed here: ' + dup.map((a) => '$' + a.toString(16).toUpperCase()).join(', '));
});

test('SECTION 2d [W450/current]: the widened register is 72, and this wave\'s three-copy constructor '
