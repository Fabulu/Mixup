...
Updated 2026-08-22 after Wave 498 local verification. W496 remains the live build.
...
Keyboard P1 coin/start remain `Digit5` and `Enter`. Every mobile scheme uses one shared P1 COIN and START row. On the first Standard controller, back/select inserts P1 coin and start presses P1 START. A held controller select cannot rearm after disconnect, blur, page hide, document visibility change, or replay entry/exit until a sampled release; the next genuine press still works. Full P2 second-controller controls and explicit mappings for non-Standard controllers remain open D106 work.
