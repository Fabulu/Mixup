...
import {
  fire as fireBulletFan, fireWithResources as fireBulletWithResources,
  spawnCoreWithResources, WriteLog,
} from './bullets.js';
...
function retire82(ram, a5, descriptor) {
  if (descriptor.retirement.semantic !== 'freeEnemy') {
    throw new TypeError('type $82 retirement must preserve freeEnemy semantics');
  }
  freeEnemy(ram, a5);
}

function deathSeq82(ram, rom, a5, a6, ctx, d1, descriptor) {
  scoreKill(ram, rom, ctx, 0x42, d1, descriptor.score);  // $274AF0/$274AF2
  ctx.soundPost?.(descriptor.sound.death);               // $274AF8 jsr edition sound
  const e1 = spawnEffect(ram, ctx, 0x0d, descriptor.effectSites[0],
    descriptor.effects);                                 // $274AFE/$274B00
  ...
  const e2 = spawnEffect(ram, ctx, 0x08, descriptor.effectSites[1],
    descriptor.effects);                                 // $274B2A/$274B2E
  ...
  retire82(ram, a5, descriptor);                        // $274B64 jmp edition free
}
...
function handler82(ram, rom, a5, ctx,
  ...
  if (offScreen242684(ram, a6)) {                       // jsr $242684 / bcc $2747E2
    if (ram.u8(a5 + R.onScreen) !== 0) {
      retire82(ram, a5, resources); return;
    } // jmp edition freeEnemy
  } else {
  ...
  if (dmg !== 0) {                                     // $2747F8 bne $274812
    ...
    scoreHit(ram, ctx, a6, dmg, a6, resources.score);     // $27481C jsr edition score-hit
    // ================================================================
    ...
    if ((d4 & 0x8000) !== 0) {                         // $27484C tst.w / bmi $274AF0
      deathSeq82(ram, rom, a5, a6, ctx, dmg, resources); // $274850 bmi.w death arm
      return;
    }
...
function fire82(ram, rom, a5, a6, ctx, descriptor) {
  ...
  spawnCues28AC72(ram, rom, a5, a6, descriptor.cues);    // $274858 jsr edition cues
  ...
  if (toHeading) {
    ...
    if (cd === 0) {                                    // $2749B8 bcc $274A22
      ...
      if (!sel.carry) {                                // $2749E8 bpl $274A22
        const dir = aim64(aimTables(rom, descriptor),
        ...
        ram.setU32(a5 + R.rec28,                       // $274A1C move.l (A3,D1.w)
          rom.u32(descriptor.aimSprite + ((nf & 0x3e) * 2))); // edition sprite table
      }
    }
  }
  draw82(ram, rom, a5, a6, descriptor);                  // $274A22
  ...
  secondFire82(ram, rom, a5, a6, ctx, descriptor);       // $274A9C..$274AEE
}
 ...
 */
function secondFire82(ram, rom, a5, a6, ctx, descriptor) {
  ...
  ctx.bulletSpawn?.(bullet.site, result);
...
function draw82(ram, rom, a5, a6, descriptor) {
  // $274A22 move.l #$60005000,D6 / $274A28 jsr $23DBCA -- the ZOOMING enqueue,
  // bucket read out of the cartridge by `resolveZoomStub`.
  enqueueZoomedThroughStub(ram, rom, descriptor.emitters.zoom, a6, 0x60005000,
    descriptor.emitters.zoomScale);
  ...
  const d1 = (((u16((pos >>> 16) + 0xfc40) << 16) | u16((pos & 0xffff) + 0xfc00))
    >>> 0);                                            // $274A32/$274A38
  enqueueRegistersThroughStub(ram, rom, descriptor.emitters.heading, d1,
  ...
  const d1b = (((u16((pos >>> 16) + 0xe200) << 16) | u16((pos & 0xffff) + 0x300))
    >>> 0);                                            // $274A64/$274A6A
  enqueueRegistersThroughStub(ram, rom, descriptor.emitters.alternate, d1b,
    descriptor.alternateSprite,                          // edition immediate sprite
...
function handler85(ram, rom, a5, ctx,
  descriptor = BLACK_WORLD_RESOURCES.enemyTypes[0x85]) {
  const { tables, unported: u } = ctx;
  const a6 = ram.u32(a5 + 0x06);
  ...
  if (offScreen2426A4(ram, a6)) {
    ...
    if (ram.u8(a5 + R.onScreen) !== 0) { retire85(ram, a5, descriptor); return; } // $275928 jmp $263762
  } else {
  ...
  if (dmg === 0) {                                     // $275946 bne $275960
    ...
    scoreHit(ram, ctx, a6, dmg, a6, descriptor.score); // $27596A jsr $286096 (W34)
    ...
    if ((ram.u16(a6 + S.hp) & 0x8000) !== 0) {         // $27599A tst.w / $27599E bmi
      deathSeq85(ram, rom, a5, a6, ctx, dmg, descriptor); // $275AF2
      return;
    }
  }
  ...
  spawnCues28AC72(ram, rom, a5, a6, descriptor.cues);
  ...
  if (ram.u32(G.freeze) === 0) {                       // $2759B2 bne $275A24
    ...
    if (cad === 0) {                                   // $2759BA bcc $275A24
      ...
      if (ram.u8(a5 + R.salvo) === ram.u8(a5 + R.rec21)) {
        aim85(ram, rom, a5, a6, descriptor);           // $2759D0..$275A1E
      }
    }
  }
  ...
  fire85(ram, rom, a5, a6, ctx, descriptor);           // $275AA2
}

function retire85(ram, a5, descriptor) {
  if (descriptor.retirement?.semantic !== 'freeEnemy') {
    unreached(descriptor.retirement?.entry ?? descriptor.handler,
      'type-$85 requires ordinary freeEnemy retirement');
  }
  freeEnemy(ram, a5);
}
...
function aim85(ram, rom, a5, a6,
  ...
  const dir = aim64(aimTables(rom, descriptor),
    u16(selfY + 0xf900), selfX, tgtY, tgtX);            // $2759FE
  ...
  ram.setU32(a5 + R.rec24,
    rom.u32(descriptor.aimSprite + ((nf & 0x3e) * 2))); // $275A18/$275A1E
}
...
function fireType85Bullet(ctx, entry, regs, resources) {
  if (entry !== resources.entry) {
    unreached(entry, `type-$85 bullet entry does not match its resource descriptor`);
  }
  if (resources.semantic === 'bank-a-direct') {
    return [spawnCoreWithResources(ctx, regs, 'A', resources)];
  }
  return fireBulletWithResources(ctx, entry, regs, resources);
}
...
function fire85(ram, rom, a5, a6, ctx,
  ...
  const d3 = ((rom.u32(descriptor.muzzle + idx) + 0xf9000000) >>> 0);
  const regs = {
  ...
  const res = fireType85Bullet(bulletCtx, descriptor.bullet.entry, regs,
    descriptor.bullet);
  ctx.bulletSpawn?.(descriptor.bullet.site, res);
...
function deathSeq85(ram, rom, a5, a6, ctx, d1,
  descriptor = BLACK_WORLD_RESOURCES.enemyTypes[0x85]) {
  const u = ctx.unported;
  scoreKill(ram, rom, ctx, 0x25, d1, descriptor.score); // $275AF2/$275AF4
  // $275AFA moveq #$0,D0 / $275AFC cmpi.b #$86,($C,A5) / $275B04 moveq #$8,D0
  const d0 = ram.u8(a5 + 0x0c) === descriptor.items.alternateType
    ? descriptor.items.alternateKind : descriptor.items.kind;
  spawnItem(ram, rom, ctx, d0, a6, descriptor.items.sites.first,
    descriptor.items);                                 // $275B06 jsr $27E812
  // $275B0C tst.w $81308C / bne $275B20 ; $275B14 cmpi.w #$8,D0 / beq $275B20
  if (ram.u16(0x81308c) === 0 && d0 !== 8) {
    spawnItem(ram, rom, ctx, d0, a6, descriptor.items.sites.second,
      descriptor.items);                               // $275B1A jsr $27E812
  }
  ...
  {
    const e1 = spawnEffect(ram, ctx, 0x05, descriptor.effectSites.first,
      descriptor.effects);                             // $275B20/$275B22
    ...
    const e2 = spawnEffect(ram, ctx, 0x0c, descriptor.effectSites.second,
      descriptor.effects);                             // $275B4C/$275B4E
    ...
    const e3 = spawnEffect(ram, ctx, 0x84, descriptor.effectSites.third,
      descriptor.effects);                             // $275B72/$275B76
  ...
  ctx.soundPost?.(descriptor.sound.death);          // WAVE A: SFX id=1, death burst          // $275BA0 jsr $28C274
  retire85(ram, a5, descriptor);                       // $275BA6 jmp $263762
}
...
function retire89(ram, a5, descriptor) {
  if (descriptor.retirement.semantic !== 'freeEnemy') {
    throw new TypeError('type $89 retirement must preserve freeEnemy semantics');
  }
  freeEnemy(ram, a5);
}
...
function handler89(ram, rom, a5, ctx,
  ...
  if (offScreen242684(ram, a6)) {                       // $277344 jsr $242684 / bcc
    if (ram.u8(a5 + R.onScreen) !== 0) { retire89(ram, a5, resources); return; }
  } else {
    ram.setU8(a5 + R.onScreen, 1);                     // $27735A
  }
  // $277360: the damage branch. The no-damage arm also computes D0, and both
  // arms converge on the palette write, so the palette is written every frame.
  let d0;
  if ((ram.u8(a6) & 0x5c) === 0) {
    d0 = ram.u8(a5 + R.cooldown);
    if (ram.u16(a6 + S.hp) < 0x120 && ram.u16(G.ca) === 0) d0 = 0x19;
  } else {
    const d1 = hitMask(ram, a6);
    ram.setU8(a6, ram.u8(a6) & 0xa3);
    scoreHit(ram, ctx, a6, d1, a6, resources.score);
    d0 = ram.u8(a6 + S.palette);
    if (d0 === 0x19) d0 = ram.u8(a5 + R.cooldown);
    d0 ^= ram.u8(a5 + R.cooldownReload);
    if ((ram.u16(a6 + S.hp) & 0x8000) !== 0) {
      deathSeq89(ram, rom, a5, ctx, d1, resources);
      return;
    }
  }
  ram.setU8(a6 + S.palette, d0 & 0xff);
  if (ram.u32(G.freeze) !== 0) { emit89(ram, rom, a6, resources); return; }
  const c = ram.u8(a5 + R.rec1E);
  ram.setU8(a5 + R.rec1E, (c - 1) & 0xff);
  if (c === 0) {
    ram.setU8(a5 + R.rec1E, ram.u8(a5 + 0x1f));
    if (!playerDist268018(ram, rom, a6, resources.fireGate).carry
        && ram.u8(a5 + R.rec1C) === ram.u8(a5 + R.rec1D)) {
      let p0 = AIM.selP1, p1 = AIM.selP2;
      if (ram.u8(a5 + 0x03) !== 0) { p0 = AIM.selP2; p1 = AIM.selP1; }
      let ok = true;
      if ((ram.u16(p0) & 0x8000) === 0) {
        if ((ram.u16(p1) & 0x8000) === 0) ok = false;
        else { const t = p0; p0 = p1; p1 = t; }
      }
      if (ok) {
        const dir = aim64(aimTables(rom, resources), ram.u16(a6 + 0x02),
          ram.u16(a6 + 0x04), ram.u16(p0 + 2), ram.u16(p0 + 4));
        const nf = slew64(ram.u16(a5 + 0x20), dir);
        ram.setU16(a5 + 0x20, nf);
        ram.setU32(a6 + S.sprite0a,
          rom.u32(resources.headingArt + u16((nf & 0x3e) * 2)));
      }
    }
  }
  if (i16(ram.u16(a6 + 0x02)) >= 0x1000) {
    const f = ram.u8(a5 + 0x1a);
    ram.setU8(a5 + 0x1a, (f - 1) & 0xff);
    if (f === 0) {
      ram.setU8(a5 + 0x1a, ram.u8(a5 + 0x17));
      if (!playerDist268018(ram, rom, a6, resources.fireGate).carry) {
        fire89(ram, rom, a5, a6, ctx, resources);
        const s = ram.u8(a5 + R.rec1C);
        ram.setU8(a5 + R.rec1C, (s - 1) & 0xff);
        if (s === 0) {
          ram.setU8(a5 + R.rec1C, ram.u8(a5 + R.rec1D));
          ram.setU8(a5 + 0x1a, u16(0x40 - ram.u16(G.b6) + 8) & 0xff);
        }
      }
    }
  }
  emit89(ram, rom, a6, resources);
}

function emit89(ram, rom, a6, descriptor) {
  const idx = u16(ram.u16(a6 + S.anim) * 4);
  if (idx >= descriptor.emitter.entries * 4) {
    unreached(descriptor.emitter.dispatch + idx,
      `type $89 emitter index $${idx.toString(16).toUpperCase()} exceeds its descriptor`);
  }
  const stub = rom.u32(descriptor.emitter.dispatch + idx);
  enqueueThroughStub(ram, rom, stub, a6);
}

function fire89(ram, rom, a5, a6, ctx, descriptor) {
  const d1 = ram.u16(a5 + 0x20);
  const off = u16(u16((d1 & 0x3e) * 2) * 2);
  const bullet = descriptor.bullet;
  if (bullet.semantic !== 'bank-a-direct') {
    throw new TypeError('type $89 fire requires bank-a-direct bullet semantics');
  }
  const regs = { d0: 0x6, d1, d2: ram.u32(a6 + 0x02),
    d3: rom.u32(descriptor.pairedFan + off), d4: 0, d5: 0, a5 };
  const ctxB = { ram, rom, log: new WriteLog(ram), mut: ctx.mut ?? null };
  ctx.bulletSpawn?.(bullet.sites[0],
    [spawnCoreWithResources(ctxB, regs, 'A', bullet)]);
  regs.d3 = rom.u32(descriptor.pairedFan + off + 4);
  ctx.bulletSpawn?.(bullet.sites[1],
    [spawnCoreWithResources(ctxB, regs, 'A', bullet)]);
}

function deathSeq89(ram, rom, a5, ctx, d1, descriptor) {
  const a6 = ram.u32(a5 + 0x06);
  scoreKill(ram, rom, ctx, 0x34, d1, descriptor.score);
  ctx.soundPost?.(descriptor.sound.death);
  noteEffect(ctx.unported, descriptor.effects.poolCEntry, a5, 'D0=$8 secondary');
  allocPoolAWithResources(ram, rom, ctx, descriptor.poolAKind, 0,
    ram.u16(a6 + 0x1e), a6, descriptor.poolA);
  const e = effectArmShared278320(
    ram, rom, ctx, a6, descriptor.effect.kind, descriptor.effect.site,
    descriptor.effect.remap, descriptor.effects,
  );
  ram.setU16(e + B.sub12, 0x0001);
  ram.setU16(e + B.sub14, 0x0000);
  ram.setU16(e + B.nudge, 0xfe00);
  ram.setU16(e + B.nudge + 2, 0x0000);
  ram.setU16(e + B.hook, descriptor.effect.hook);
  retire89(ram, a5, descriptor);
}
...
const HANDLERS = new Map([
  ...
  [0x2747c6, (ram, rom, a5, ctx) =>
    handler82(ram, rom, a5, ctx, BLACK_WORLD_RESOURCES.enemyTypes[0x82])],
...
export function handlerMap(resources = BLACK_WORLD_RESOURCES) {
  ...
  for (const descriptor of Object.values(resources.enemyTypes ?? {})) {
    if (descriptor.algorithm === 'type0D') {
      ...
      handlers.set(descriptor.handler, (ram, rom, a5, ctx) =>
        handler85(ram, rom, a5, ctx, descriptor));
    }
  }
