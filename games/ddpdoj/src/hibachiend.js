...
// =============================================================== A0 MAIN SCRIPT 7
// `$2A4E8E` entry 7 is {$2A524E, $2A5262}. The inherited speed is deliberate:
// this phase halves a signed random heading byte, falls through, and keeps roaming.

/** `$2A524E`. Clear target state and seed heading with signed `asr.b #1`. */
export function main7Init2A524E(ram, rom, ctx, a4) {
  const a6 = bossA6(ctx, HIBACHI_A0.s7Init);
  ram.setU8(a4, 0);                                          // $2A524E/$2A5250
  ram.setU8(a4 + 0x01, 0);                                   // $2A5252
  const raw = drawByte242B3C(ram, rom);                       // $2A5256
  const signed = raw >= 0x80 ? raw - 0x100 : raw;
  ram.setU8(a6 + 0x1b, (signed >> 1) & 0xff);                 // $2A525C asr.b #1
}

/** `$2A5262`. Move, slew, draw a replacement target, and refresh all parts. */
export function main7Step2A5262(ram, rom, ctx, a4) {
  const a6 = bossA6(ctx, HIBACHI_A0.s7Step);
  applyVelocityA6(ram, ctx.tables, a6);                       // $2A5262

  if (ram.u8(a4 + 0x01) !== 0) {                              // $2A5268
    const target = ram.u8(a4);
    const heading = slew64FromRecord(ram, a6, target);         // $2A526E/$2A5270
    ram.setU8(a6 + 0x1b, heading);                            // $24217E's store
    if (heading === target) ram.setU8(a4 + 0x01, 0);           // $2A5276..$2A527C
  }

  let target = drawByte242B3C(ram, rom);                      // $2A527E
  const x = u16(ram.u16(a6 + 0x04) + 0x2800);                 // $2A5284/$2A5288
  if (x < 0x4200) {                                           // unsigned BCC boundary
    target = (target + 0x10) & 0xff;                          // $2A5292
  } else if (x >= 0x4600) {                                   // unsigned BCS boundary
    target = (target + 0x30) & 0xff;                          // $2A529E
  } else {
    const y = ram.u16(a6 + 0x02);                             // $2A52A4 swap D1
    if (y >= 0x7b80 && y < 0x7c80) {                          // $2A52A6..$2A52B0
      placeMain0Parts2A4EB6(ram, a6);                        // $2A52C2 bra.w
      return;
    }
    if (y >= 0x7c80) target = (target + 0x20) & 0xff;         // $2A52B2
  }
...
// =============================================================== A0 MAIN SCRIPT 9
// `$2A4E9E` entry 9 is {$2A5338, $2A5340}. The init preserves inherited speed
// and heading, falls through, and uses the narrower centre classifier for this phase.

/** `$2A5338`. Clear both target-state bytes, then fall through into the step. */
export function main9Init2A5338(ram, a4) {
  ram.setU8(a4, 0);                                          // $2A5338/$2A533A
  ram.setU8(a4 + 0x01, 0);                                   // $2A533C
}

/** `$2A5340`. Move, slew, draw a replacement target, and refresh all parts. */
export function main9Step2A5340(ram, rom, ctx, a4) {
  const a6 = bossA6(ctx, HIBACHI_A0.s9Step);
  applyVelocityA6(ram, ctx.tables, a6);                       // $2A5340

  if (ram.u8(a4 + 0x01) !== 0) {                              // $2A5346
    const target = ram.u8(a4);
    const heading = slew64FromRecord(ram, a6, target);         // $2A534C/$2A534E
    ram.setU8(a6 + 0x1b, heading);                            // $24217E's store
    if (heading === target) ram.setU8(a4 + 0x01, 0);           // $2A5354..$2A535A
  }

  let target = drawByte242B3C(ram, rom);                      // $2A535C
  const x = u16(ram.u16(a6 + 0x04) + 0x2800);                 // $2A5362/$2A5366
  if (x < 0x4000) {                                           // unsigned BCC boundary
    target = (target + 0x10) & 0xff;                          // $2A5370
  } else if (x >= 0x4800) {                                   // unsigned BCS boundary
    target = (target + 0x30) & 0xff;                          // $2A537C
  } else {
    const y = ram.u16(a6 + 0x02);                             // $2A5382 swap D1
    if (y >= 0x6200 && y < 0x6600) {                          // $2A5384..$2A538E
      placeMain0Parts2A4EB6(ram, a6);                        // $2A53A0 bra.w
      return;
    }
    if (y >= 0x6600) target = (target + 0x20) & 0xff;         // $2A5390
  }
  ram.setU8(a4, target & 0x3f);                               // $2A5394/$2A5398
  ram.setU8(a4 + 0x01, 1);                                   // $2A539A
  placeMain0Parts2A4EB6(ram, a6);                            // $2A53A0 bra.w
}
...
registerScript(HIBACHI_A0.s6Step,
  (ram, rom, ctx, a4) => main6Step2A51EA(ram, rom, ctx, a4));
registerScript(HIBACHI_A0.s7Init, initThenStep(
  (ram, rom, ctx, a4) => main7Init2A524E(ram, rom, ctx, a4),
...
registerScript(HIBACHI_A0.s9Step,
  (ram, rom, ctx, a4) => main9Step2A5340(ram, rom, ctx, a4));
