...
/**
 * `$260A20` -- THE ANNOUNCE-MAILBOX SIDE SELECTOR.  Four instructions.  W344.
 *
 *     260a20  lea $813162,A4        side 0
 *     260a26  tst.b D0 / beq $260A32
 *     260a2c  lea $813166,A4        side 1
 *     260a32  rts
 *
 * The pair this file's own `announce260B30` docstring already names (`A4 = $813162`/`$813166`). Note the
 * test is `tst.b`, so **only the LOW BYTE of D0 selects** -- a caller passing `$0100` gets side 0.
 *
 * @returns {number} the mailbox base for the requested side
 */
export function announceMailbox260A20(d0) {
  return (d0 & 0xff) !== 0 ? 0x813166 : 0x813162;          // $260A26 tst.b / $260A2C
}

/**
 * `$260A88` -- POST ANNOUNCE REQUEST 1.  Five instructions, **SIX callers**.  W344.
 *
 *     260a88  move.l A4,-(A7)
 *     260a8a  bsr $260A20            pick the side's mailbox
 *     260a8c  move.w #$1,(A4)        request := 1
 *     260a90  move.w #$0,($2,A4)     state   := 0
 *     260a96  movea.l (A7)+,A4 / rts
 *
 * **The same `(request, state)` mailbox shape as `tallyRequest25FF38`**, which posts into `$8130FA`/`$81311E`
 * the same way -- write the request word and clear the state word. Two independent mailboxes in this game use
 * that convention, so treat "word 0 is a request, word 1 is a state, and posting clears the state" as the
 * house pattern rather than a coincidence of two routines.
 *
 * The transition screen's phase-0 arm calls it at `$25DC8A` with `D0 = ($7,A5)` -- the side -- immediately
 * after advancing the phase to 1. So advancing the tally phase ANNOUNCES on the same frame.
 */
export function postAnnounce260A88(ram, d0) {
  const a4 = announceMailbox260A20(d0);                    // $260A8A bsr $260A20
  ram.setU16(a4 + 0x00, 1);                                // $260A8C move.w #$1,(A4)
  ram.setU16(a4 + 0x02, 0);                                // $260A90 move.w #$0,($2,A4)
  return a4;
}
